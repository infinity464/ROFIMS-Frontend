...
@Component({
    selector: 'app-personal-qualification',
    imports: [DynamicFormComponent, DataTable,   Fluid],
    templateUrl: './personal-qualification.html',
    providers: [],
    styleUrl: './personal-qualification.scss'
})
export class PersonalQualification {
    ...
    codeType: string = 'PersonalQualification';
    title: string = 'Personal Qualification';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Personal Qualification Name (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Personal Qualification Name (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Personal Qualification Name (EN)' },
            { field: 'codeValueBN', header: 'Personal Qualification Name (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'PersonalQualification created successfully'
                });
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create personal-qualification'
                });
                this.isSubmitting = false;
            }
        });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'PersonalQualification updated successfully'
                });
                this.isSubmitting = false;
            },
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update personal-qualification'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Personal Qualification deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete personal-qualification'
                        });
                    }
                });
            }
