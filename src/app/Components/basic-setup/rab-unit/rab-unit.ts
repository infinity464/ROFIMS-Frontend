...
@Component({
    selector: 'app-rab-unit',
    imports: [DynamicFormComponent, DataTable, ConfirmDialog, Toast, Fluid],
    templateUrl: './rab-unit.html',
    providers: [MessageService, ConfirmationService],
    styleUrl: './rab-unit.scss'
})
export class RabUnit {
    codeType: string = 'RabUnit';
    title: string = 'Rab Unit';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Rab Unit Name (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Rab Unit Name (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Rab Unit Name (EN)' },
            { field: 'codeValueBN', header: 'Rab Unit Name (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'RabUnit created successfully'
                });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Rab Unit updated successfully'
                });
                this.isSubmitting = false;
            },
            error: (err) => {
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Rab Unit deleted successfully'
                        });
                    },
                    error: (err) => {
