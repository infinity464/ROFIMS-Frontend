...
@Component({
    selector: 'app-punishment-type',
    imports: [DynamicFormComponent, DataTable,   Fluid],
    templateUrl: './punishment-type.html',
    providers: [],
    styleUrl: './punishment-type.scss'
})
export class PunishmentType {
    ...
    codeType: string = 'PunishmentType';
    title: string = 'Punishment Type';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Punishment Type Name (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Punishment Type Name (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Punishment Type Name (EN)' },
            { field: 'codeValueBN', header: 'Punishment Type Name (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Punishment Type created successfully'
                });
            },
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create punishment-type'
                });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update punishment-type'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Punishment Type deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete punishment-type'
                        });
                    }
                });
            }
