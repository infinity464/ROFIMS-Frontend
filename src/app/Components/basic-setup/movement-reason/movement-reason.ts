...
@Component({
    selector: 'app-movement-reason',
    imports: [DynamicFormComponent, DataTable, Fluid],
    templateUrl: './movement-reason.html',
    providers: [],
    styleUrl: './movement-reason.scss'
})
export class MovementReason {
    ...
    codeType: string = 'MovementReason';
    title: string = 'Movement Reason';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Movement Reason (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Movement Reason (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Movement Reason (EN)' },
            { field: 'codeValueBN', header: 'Movement Reason (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            next: () => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Movement Reason created successfully'
                });
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create movement reason'
                });
                this.isSubmitting = false;
            }
        });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            next: () => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Movement Reason updated successfully'
                });
                this.isSubmitting = false;
            },
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update movement reason'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Movement Reason deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete movement reason'
                        });
                    }
                });
            }
