...
export class CourseGrade {
    ...
    codeType: string = 'CourseGrade';
    title: string = 'Course Grade';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Course Grade Name (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Course Grade Name (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Course Grade Name (EN)' },
            { field: 'codeValueBN', header: 'Course Grade Name (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create course-grade'
                });
                this.isSubmitting = false;
            }
        });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update course-grade'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Course Grade deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete course-grade'
                        });
                    }
                });
            }
