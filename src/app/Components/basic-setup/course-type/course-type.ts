...
export class CourseType {
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Course Type Name (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Course Type Name (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Course Type Name (EN)' },
            { field: 'codeValueBN', header: 'Course Type Name (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create course-type'
                });
                this.isSubmitting = false;
            }
        });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update course-type'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'CourseType deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete course-type'
                        });
                    }
                });
            }
