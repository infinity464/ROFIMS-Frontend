...
@Component({
    selector: 'app-medical-category-type',
    imports: [DynamicFormComponent, DataTable, Fluid],
    templateUrl: './medical-category-type.html',
    providers: [],
    styleUrl: './medical-category-type.scss'
})
export class MedicalCategoryType {
    ...
    codeType: string = 'MedicalCategoryType';
    title: string = 'Medical Category Type';
    ...
    formConfig: FormConfig = {
        formFields: [
            {
                name: 'codeValueEN',
                label: 'Medical Category (English)',
                type: 'text',
                required: true
            },
            {
                name: 'codeValueBN',
                label: 'Medical Category (Bangla)',
                type: 'text',
                required: true
            },
            {
    ...
    tableConfig: TableConfig = {
        tableColumns: [
            { field: 'codeValueEN', header: 'Medical Category (EN)' },
            { field: 'codeValueBN', header: 'Medical Category (BN)' },
            {
                field: 'status',
                header: 'Status',
                type: 'boolean',
        ...
        this.masterBasicSetupService.create(createPayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Medical Category Type created successfully'
                });
            },
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to create medical category type'
                });
        ...
        this.masterBasicSetupService.update(updatePayload).subscribe({
            next: (res) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Medical Category Type updated successfully'
                });
                this.isSubmitting = false;
            },
            error: (err) => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: err?.error?.message || 'Failed to update medical category type'
                });
                this.isSubmitting = false;
            }
        });
                ...
                this.masterBasicSetupService.delete(row.codeId).subscribe({
                    next: () => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Success',
                            detail: 'Medical Category Type deleted successfully'
                        });
                    },
                    error: (err) => {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: err?.error?.message || 'Failed to delete medical category type'
                        });
                    }
                });
            }
