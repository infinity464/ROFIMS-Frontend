...
import { EquivalentNameVacancyDistributionModel, EquivalentNameVacancyLockModel } from '../shared/models/equivalent-name-vacancy';

export interface EqvColumn {
    field: string;
    header: string;
    equivalentNameId: number;
}

/** One display row: office label, indent level, quantities per equivalent name, and a row total */
export interface DisplayRow {
    rabCodeId: number;
    /** 0=Unit, 1=Wing, 2=Branch, 3=Sub-Branch, 4=Section, 5=Sub-Section */
    level: 0 | 1 | 2 | 3 | 4 | 5;
    officeName: string;
    /** This office's OWN directly-assigned quantity per field (no descendants). */
    ownByField: Record<string, number>;
    /** Displayed quantity per field = own + sum of all descendants (roll-up). */
...
@Component({
    selector: 'app-equivalent-name-vacancy',
    standalone: true,
    ...
    templateUrl: './equivalent-name-vacancy.html',
    styleUrl: '../vacancy-distribution-summary/vacancy-distribution-summary.scss'
})
export class EquivalentNameVacancyComponent implements OnInit {
    ...
    private lastDistributionList: EquivalentNameVacancyDistributionModel[] = [];
    ...
    /** Equivalent-name options for modal dropdown */
    get eqvOptionsForModal(): { codeId: number; label: string }[] {
        return this.dynamicColumns.map((c) => ({ codeId: c.equivalentNameId, label: c.header }));
    }
    ...
    ngOnInit(): void {
        ...
        this.distributionForm = this.fb.group({
            equivalentNameId: [null as number | null, Validators.required],
    ...
    saveDistributionModal(): void {
        ...
        const model: EquivalentNameVacancyDistributionModel = {
            orgId: this.selectedOrg.orgId,
            equivalentNameId: val.equivalentNameId,
            rabCodeId,
        ...
        const existing = this.lastDistributionList.find(
            (d) =>
                d.orgId === model.orgId &&
                d.equivalentNameId === model.equivalentNameId &&
        ...
        const obs = model.id != null
            ? this.masterBasicSetup.updateEquivalentNameVacancyDistribution(model)
            : this.masterBasicSetup.saveEquivalentNameVacancyDistribution(model);
        obs.subscribe({
            next: (res) => {
     ...
     */
    cellBreakdown(row: DisplayRow, col: EqvColumn): { own: number; children: number; total: number } | null {
    ...
    onCellEnter(row: DisplayRow, col: EqvColumn, event: MouseEvent): void {
    ...
    toggleLock(row: DisplayRow, event: Event): void {
        ...
        const model: EquivalentNameVacancyLockModel = {
            orgId: this.selectedOrg.orgId,
        ...
        this.masterBasicSetup.setEquivalentNameVacancyLock(model).subscribe({
            next: (res) => {
    ...
    startEdit(row: DisplayRow, col: EqvColumn, event: Event): void {
        if (!this.selectedOrg) return;
    ...
    onCellKeydown(event: KeyboardEvent, row: DisplayRow, col: EqvColumn): void {
        if (event.key === 'Enter') {
    ...
    onCellBlur(row: DisplayRow, col: EqvColumn): void {
        if (this.editingCellKey === this.cellKey(row, col.field)) {
            this.saveCell(row, col);
        }
    }

    private saveCell(row: DisplayRow, col: EqvColumn): void {
        ...
        const existing = this.lastDistributionList.find(
            (d) => d.orgId === this.selectedOrg!.orgId
                && d.equivalentNameId === col.equivalentNameId
                && (d.rabCodeId ?? (d as any).RABCodeId) === row.rabCodeId
        );

        const model: EquivalentNameVacancyDistributionModel = {
            orgId: this.selectedOrg.orgId,
            equivalentNameId: col.equivalentNameId,
            rabCodeId: row.rabCodeId,
            quantity: newValue,
            createdBy: this.shareService.getCurrentUser?.() ?? 'System',
            lastUpdatedBy: this.shareService.getCurrentUser?.() ?? 'System'
    ...
    private loadSummaryForOrg(orgId: number): void {
        this.loading = true;
        // Equivalent names are a GLOBAL CommonCode list (same RAB ranks for every Mother Org),
        // so load them by type only — not filtered by orgId.
        this.masterBasicSetup.getAllByType('EquivalentName').subscribe({
            next: (codes) => {
                const eqvList = this.sortCodes((codes ?? []) as CommonCode[]);
                if (eqvList.length === 0) { this.loading = false; return; }
                const eqvNameById: Record<number, string> = {};
                eqvList.forEach((r) => { eqvNameById[r.codeId] = r.codeValueEN ?? ''; });
                this.dynamicColumns = eqvList.map((r) => ({
                    field: `eqv_${r.codeId}`,
                    header: eqvNameById[r.codeId] ?? String(r.codeId),
                    equivalentNameId: r.codeId
                }));
                forkJoin({
                    distPerEqv: forkJoin(
                        eqvList.map((r) =>
                            this.masterBasicSetup.getEquivalentNameVacancyDistributionByVacancy(orgId, r.codeId)
                        )
                    ),
                    locks: this.masterBasicSetup.getEquivalentNameVacancyLocksByOrg(orgId)
                }).subscribe({
                            next: ({ distPerEqv, locks }) => {
                                const allDist: EquivalentNameVacancyDistributionModel[] = [];
                                distPerEqv.forEach((list) => allDist.push(...(list ?? [])));
    ...
    private buildDisplayRows(distributionList: EquivalentNameVacancyDistributionModel[]): void {
        /* map: rabCodeId -> { equivalentNameId -> quantity } */
        ...
        distributionList.forEach((d) => {
            ...
            entry[d.equivalentNameId] = (entry[d.equivalentNameId] ?? 0) + (d.quantity ?? 0);
        });
        ...
        source.forEach(({ codeId, level }) => {
            const qtyByEqvId = byRab.get(codeId) ?? {};
            const ownByField: Record<string, number> = {};
            let ownTotal = 0;
            this.dynamicColumns.forEach((col) => {
                const qty = qtyByEqvId[col.equivalentNameId] ?? 0;
