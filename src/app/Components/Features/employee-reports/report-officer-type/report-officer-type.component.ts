 ...
 * registry field maps to EmployeeInfo.OfficerType (CommonCode FK).
 */
@Component({
    selector: 'app-report-officer-type',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
...
export class ReportOfficerTypeComponent implements OnInit, OnChanges {
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportOfficerTypeComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportOfficerTypeComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportOfficerTypeComponent.NAME_EXTRAS_KEY)) {
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportOfficerTypeComponent.MEMBER_STATUS_LABELS[raw];
    ...
    get rabSectionTitle(): string {
        ...
        return this.L[this.lang]['report.title.officerType'];
    }
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.officerType'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `officer-type-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `officer-type-report_${this.lang}.xlsx`);
    }
...
<body>
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportOfficerTypeComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
