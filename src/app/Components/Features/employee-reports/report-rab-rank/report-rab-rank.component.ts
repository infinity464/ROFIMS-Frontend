 ...
 * (universal RAB rank tier). The backend's `rabRankEquivalent` sentinel
 * resolves it via RankEquivalent into the matching per-org MotherOrgRanks
 * with an EXISTS subquery — no per-rank fan-out needed in the frontend.
 *
 * Filter panel mirrors the member-appointment / officer-type layout:
 * Mother Org, Rank, Corps, Trade — independent of the parent-locked tier.
 */
@Component({
    selector: 'app-report-rab-rank',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
...
export class ReportRabRankComponent implements OnInit, OnChanges, OnDestroy {
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportRabRankComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportRabRankComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportRabRankComponent.NAME_EXTRAS_KEY)) {
    ...
    get rabSectionTitle(): string {
        ...
        return this.L[this.lang]['report.title.rabRank'];
    }
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.rabRank'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                switch (col.hint) {
                    ...
                    case 'SelectedValue': return new TableCell({ ...cellOpts, children: [new Paragraph({ children: [run(this.selectedValueCellText)] })] });
                    case 'MemberStatus': return new TableCell({ ...cellOpts, children: [new Paragraph({ children: [run(this.memberStatusText(row))] })] });
                    case 'BlankRemark':
        ...
        saveAs(blob, `rab-rank-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `rab-rank-report_${this.lang}.xlsx`);
    }
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportRabRankComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportRabRankComponent.MEMBER_STATUS_LABELS[raw];
    ...
    load(): void {
        ...
        // Parent-locked RAB rank tier (EquivalentName CodeId) drives the
        // `rabRankEquivalent` sentinel — backend resolves it via the
        // RankEquivalent table into the matching per-org MotherOrgRanks
        // with an EXISTS subquery. Mother Org / Rank / Corps / Trade are
        // additive user picks, AND'd with the rank-tier match.
        const criteria: DynamicReportCriterion[] = [];
        if (this.commonCodeId != null && this.commonCodeId > 0)
            criteria.push({ fieldKey: 'rabRankEquivalent', idValue: this.commonCodeId });
