 ...
 *
 * Filter context: parent's `commonCodeId` is the selected MOTHER ORG.
 * The internal Mother Org picker is therefore not shown — only the
 * dependent pickers (Mother Org Unit, Rank, Corps, Trade) appear.
 *
 * Backend: runs against the dynamic-report engine
 * (`runDynamicEmployeeBaseReport`) so:
 *   - the column picker can surface ANY field in ReportFieldRegistry,
 *   - access control + posting-status filtering go through the same
 *     code path as every other employee report,
 *   - sort order is the registry default (org → rank → service id).
 */
@Component({
...
export class ReportMotherOrgComponent implements OnInit, OnChanges, OnDestroy {
    ...
    selectedMotherOrgUnitIds: number[] = [];
    ...
    // ── Column picker ────────────────────────────────────────────────
    /**
     * Same catalog shape as member-appointment / batch-course. Keys for
     * Plain-hint columns match the dynamic backend's FieldKeys so the
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportMotherOrgComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter(k => k !== ReportMotherOrgComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
     ...
     * Professional Qualification and Corps. An "N/A" corps is skipped.
     */
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportMotherOrgComponent.NAME_EXTRAS_KEY)) {
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportMotherOrgComponent.MEMBER_STATUS_LABELS[raw];
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        {
            const ids = this.selectedMotherOrgUnitIds;
            if (ids.length) {
                const names = ids
                    .map((id) => this.motherOrgUnitOptions.find((u) => u.orgId === id))
                    .filter((u): u is MotherOrganizationModel => u != null)
                    .map((u) => (this.lang === 'bn' ? (u.orgNameBN || u.orgNameEN) : u.orgNameEN))
                    .filter((v): v is string => !!v);
                if (names.length) items.push({ label: L['report.search.motherOrgUnit'] ?? 'Mother Org Unit', value: names.join(', ') });
            }
        }
        ...
        multi(this.selectedMemberTypeIds, this.memberTypeOptions, this.lang === 'bn' ? 'সদস্য ধরন' : 'Member Type');
        {
            ...
            if (ids.length) {
                const names = ids
                    ...
                    .map((u) => (this.lang === 'bn' ? (u.orgNameBN || u.orgNameEN) : u.orgNameEN))
                    .filter((v): v is string => !!v);
                const label = L['report.search.motherOrgUnit'] ?? 'Mother Org Unit';
                if (names.length) lines.push(`${label}: ${names.join(', ')}`);
            }
        }
        multi(this.selectedRankIds, this.rankOptions, L['report.search.rank']);
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `mother-org-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `mother-org-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        // Coerce to string before HTML-escaping — backend fields spread in
        // via `...d` in adaptDynamicRow can arrive as numbers and would
        // throw "replace is not a function" otherwise.
...
<body>
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportMotherOrgComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['commonCodeId']) {
            // Reset dependent picks + options whenever the parent's Mother
            // Org changes — old Rank / Corps / Trade lists no longer apply.
            ...
            if (id != null) this.loadDependentOptions(id);
    ...
    /** Load Mother Org Unit / Rank / Corps options scoped to the selected
        Mother Org (parent's commonCodeId). Trade is cascaded off Corps. */
    private loadDependentOptions(orgId: number): void {
        ...
        this.commonCodeService.getAllActiveCommonCodesByOrgIdAndType(orgId, 'Corps').subscribe({
            next: (codes: CommonCodeModel[]) => (this.corpsOptions = this.mapCodes(codes)),
        });
    }
    ...
    onFilterChange(): void {
        // Search runs only when user clicks Search.
    }
    ...
    get activeFilterCount(): number {
        ...
        if (this.selectedMotherOrgUnitIds.length > 0) c++;
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            if (key === 'callNoRankName') { push('prefix'); push('serviceId'); push('armyRank'); push('nameEnglish'); push('nameBangla'); push('awards'); push('professionalQualification'); push('corps'); continue; }
    ...
    load(): void {
        ...
        // Parent's commonCodeId drives the `motherOrganization` filter;
        // every other pick is additive. Keys MUST match ReportFieldRegistry
        // FieldKeys — unknown keys 400 at the whitelist.
        const criteria: DynamicReportCriterion[] = [];
        if (this.commonCodeId != null && this.commonCodeId > 0)
            criteria.push({ fieldKey: 'motherOrganization', idValue: this.commonCodeId });
        if (this.selectedMemberTypeIds.length > 0)
            criteria.push({ fieldKey: 'memberType', idValues: this.selectedMemberTypeIds });
        if (this.selectedMotherOrgUnitIds.length > 0)
            criteria.push({ fieldKey: 'motherUnit', idValues: this.selectedMotherOrgUnitIds });
