...
import type { LeaveReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Leave Report (leave-application / history) — list of presently-serving members
 * who are on approved leave. Defaults to members currently on leave; the backend
 * applies the Approved-only membership + on-leave / leave-window filter, the
...
@Component({
    ...
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-leave.component.scss']
})
export class ReportLeaveComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
    ...
    leaveTypeOptions: { label: string; labelBn: string; value: number }[] = [];
     ...
     * org-tree picker. Selected node ids are sent to the backend as the
    ...
    /** Leave date-window filters. */
    leaveFromDate: Date | null = null;
    leaveToDate: Date | null = null;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: 'Serial' | 'Personnel' | 'Date' | 'Plain' | 'Remarks' | 'NameSuffix' | 'CallNoRankName'; defaultVisible: boolean }[] = [
        ...
        { key: 'addressDuringLeave', labelEN: 'Leave Address', labelBN: 'ছুটিকালীন ঠিকানা', hint: 'Plain', defaultVisible: false },
    ...
    private static readonly extraDateKeys = new Set(['leaveFrom', 'leaveTo', 'joiningInRab', 'dateOfCommission', 'dob']);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set(['rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'postingStatus', 'serviceId', 'leaveCertificateNo', 'addressDuringLeave']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportLeaveComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: LeaveReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportLeaveComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: LeaveReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: LeaveReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportLeaveComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
    ...
    private loadLeaveTypes(): void {
        this.commonCodeService.getAllActiveCommonCodesType('LeaveType').subscribe({
            next: (codes: CommonCodeModel[]) => (this.leaveTypeOptions = this.mapCodes(codes)),
            error: () => (this.leaveTypeOptions = [])
        });
    }
    ...
    get activeFilterCount(): number {
        ...
        if (this.selectedLeaveTypeIds.length > 0) c++;
        if (this.leaveFromDate != null) c++;
        if (this.leaveToDate != null) c++;
        return c;
    }
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        multi(this.selectedLeaveTypeIds, this.leaveTypeOptions, 'Leave Type', 'ছুটির ধরন');
        if (this.leaveFromDate != null) items.push({ label: this.lang === 'en' ? 'Leave From' : 'ছুটি শুরুর তারিখ', value: this.formatDateLabel(this.fmtDate(this.leaveFromDate)) });
        if (this.leaveToDate != null) items.push({ label: this.lang === 'en' ? 'Leave To' : 'ছুটি শেষের তারিখ', value: this.formatDateLabel(this.fmtDate(this.leaveToDate)) });
        return items;
    }
    ...
    /**
     * Translate display column keys into backend registry FieldKeys, expanding
     * composite columns (nameExtras, callNoRankName, serviceId, rabUnitHierarchy)
     * into every backend field they render. Plain keys fall through colKeyToBackend.
     */
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportLeaveComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    private loadPage(): void {
        ...
        if (this.selectedLeaveTypeIds.length > 0) criteria.push({ fieldKey: 'leaveType', idValues: this.selectedLeaveTypeIds });

        // Map column keys → registry FieldKeys (expanding composite columns).
        const columns = this.backendColumnKeys();

        this.reportService
            .runDynamicLeaveReport({
                columns,
                criteria,
                // No posting-status filter for full-access users — they see leave
                // across all posting statuses. Org-scope-restricted callers are
                // still locked to "Servings" server-side (RunLeaveAsync).
                postingStatusFilter: null,
                leaveDateFrom: this.fmtDate(this.leaveFromDate),
                leaveDateTo: this.fmtDate(this.leaveToDate),
            ...
            .subscribe({
                ...
                error: (err) => {
                    console.error(err);
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || 'Failed to load leave list.' });
                    this.loading = false;
                }
            });
    }
    ...
    get reportTitle(): string {
        return this.lang === 'en' ? 'List of Members on Leave' : 'ছুটিতে থাকা সদস্যের তালিকা';
    }
    ...
    personnelMetaText(row: LeaveReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: LeaveReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
                // Service ID cell shows the member's prefix before the number (e.g. "K. 4045260").
                const r = row as any;
                const prefix = this.codeValue(r.prefix, r.prefixBN);
                const svc = row.serviceId != null && (row.serviceId as any) !== '' ? this.displayNum(row.serviceId) : '';
            ...
            case 'leaveType':
                return this.codeValue(row.leaveType, row.leaveTypeBN);
            case 'leaveFrom':
                return this.formatDateLabel(row.leaveFrom);
            case 'leaveTo':
                return this.formatDateLabel(row.leaveTo);
            case 'leaveDays':
                return this.formatLeaveDuration(row.leaveDays);
            case 'rmks':
    ...
    /** Inclusive leave length in days → "N day(s)" / "N দিন". */
    private formatLeaveDuration(days: number | null | undefined): string {
        if (days == null) return '—';
        if (this.lang === 'bn') return `${BanglaNumerals.toBangla(String(days))} দিন`;
        return `${days} ${days === 1 ? 'day' : 'days'}`;
    }

    /** Remarks may be stored as rich-text HTML; render them as plain text (strip
    ...
    private extraCellValue(row: LeaveReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportLeaveComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportLeaveComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): LeaveReportRow {
        return {
            ...d,
            ser,
            ...
            leaveTypeBN: d['leaveTypeBN'] as string,
            leaveFrom: d['leaveFrom'] as string,
            leaveTo: d['leaveTo'] as string,
            leaveDays: (d['leaveDays'] as number) ?? null,
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: LeaveReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
                ...
                case 'Plain':
                default:
                    return `<td class="td-${col.key}">${esc(this.cellValue(row, col.key))}</td>`;
            }
        };
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .td-name { min-width: 34mm; }
    .td-rank { min-width: 22mm; }
    .td-corps, .td-trade { word-break: normal; overflow-wrap: normal; }
