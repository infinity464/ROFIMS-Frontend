...
import type { PresentStatusReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Employee Present-Status Report — list of members at their latest present
 * status (On Duty / Regular Posting Out / RTU on Discipline Issue / Deceased /
 * Absent / Arrested, or All). The backend applies the status filter; the
 * frontend just displays. RAB-formal layout, dynamic column picker,
 * drag-to-reorder, Print/Word/Excel exports.
 */
@Component({
    selector: 'app-report-present-status',
    standalone: true,
    ...
    templateUrl: './report-present-status.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-present-status.component.scss']
})
export class ReportPresentStatusComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
    ...
    list: PresentStatusReportRow[] = [];
    loading = false;
    searched = false;

    /** Primary control — present-status report type (a specific type is required). */
    selectedPresentStatus: string | null = null;
    presentStatusOptions = [...PresentStatusTypeOptions];

    /** Type-specific date filters. statusDate = the event date (death/absent/RTU/…);
        release dates apply only to Regular Posting Out. */
    statusDateFrom: Date | null = null;
    statusDateTo: Date | null = null;
    releaseFrom: Date | null = null;
    releaseTo: Date | null = null;

    /** Dynamic label for the status (event) date, per selected type. */
    get statusDateLabel(): string {
        switch (this.selectedPresentStatus) {
            case 'Deceased': return 'Date of Death';
            case 'Absent': return 'Absent Date';
            case 'Arrested': return 'Arrest Date';
            case 'RegularPostingOut': return 'Posting Out Date';
            case 'RTUOnDisciplineIssue': return 'RTU Date';
            default: return 'Status Date';
        }
    }
    /** Release/reduce date range applies only to Regular Posting Out. */
    get showReleaseDates(): boolean {
        return this.selectedPresentStatus === 'RegularPostingOut';
    }
    private fmtDate(d: Date | null): string | null {
        if (!d) return null;
        return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    }
    /** Clear the type-specific date filters whenever the report type changes. */
    onPresentStatusChange(): void {
     ...
     * shared org-tree picker. Selected node ids are sent to the backend as the
    ...
    /** Resolve a present-status enum value to its display label. */
    statusLabel(value: string | null | undefined): string {
        if (value == null || value === '') return '—';
        const opt = PresentStatusTypeOptions.find((o) => String(o.value) === String(value));
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: 'Serial' | 'Personnel' | 'Date' | 'Plain' | 'Remarks' | 'NameSuffix' | 'CallNoRankName'; defaultVisible: boolean }[] = [
        ...
        { key: 'deceasedReason', labelEN: 'Deceased Reason', labelBN: 'মৃত্যুর কারণ', hint: 'Plain', defaultVisible: false },
        { key: 'deceasedPlace', labelEN: 'Deceased Place', labelBN: 'মৃত্যুর স্থান', hint: 'Plain', defaultVisible: false },
        { key: 'absentType', labelEN: 'Absent Type', labelBN: 'অনুপস্থিতির ধরন', hint: 'Plain', defaultVisible: false },
        { key: 'absentReport', labelEN: 'Absent Report', labelBN: 'অনুপস্থিতির প্রতিবেদন', hint: 'Plain', defaultVisible: false },
    ...
    private static readonly extraDateKeys = new Set(['statusDate', 'joiningInRab', 'dob', 'dateOfRelease', 'reduceFromRab']);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set(['rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'serviceId', 'statusDetails', 'rtuCause', 'deceasedReason', 'deceasedPlace', 'absentReport', 'incidentDetails', 'inquiryReport']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportPresentStatusComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: PresentStatusReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportPresentStatusComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: PresentStatusReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: PresentStatusReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportPresentStatusComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
    ...
    get activeFilterCount(): number {
        let c = 0;
        if (this.selectedPresentStatus) c++;
        if (this.statusDateFrom) c++;
        if (this.statusDateTo) c++;
        if (this.showReleaseDates && this.releaseFrom) c++;
        if (this.showReleaseDates && this.releaseTo) c++;
    ...
    filterSubtitle(): string {
        // Keep the Search title/subtitle in English regardless of the selected language.
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        if (this.selectedPresentStatus) {
            const opt = this.presentStatusOptions.find((o) => o.value === this.selectedPresentStatus);
            items.push({ label: this.lang === 'en' ? 'Report Type' : 'প্রতিবেদন ধরন', value: opt ? opt.label : this.selectedPresentStatus });
        }
        if (this.statusDateFrom) items.push({ label: `${this.statusDateLabel} From`, value: this.formatDateLabel(this.fmtDate(this.statusDateFrom)) });
        if (this.statusDateTo) items.push({ label: `${this.statusDateLabel} To`, value: this.formatDateLabel(this.fmtDate(this.statusDateTo)) });
        if (this.showReleaseDates && this.releaseFrom) items.push({ label: 'Date of Release From', value: this.formatDateLabel(this.fmtDate(this.releaseFrom)) });
        if (this.showReleaseDates && this.releaseTo) items.push({ label: 'Date of Release To', value: this.formatDateLabel(this.fmtDate(this.releaseTo)) });
    ...
    clearFilters(): void {
        this.selectedPresentStatus = null;
        this.statusDateFrom = null;
        this.statusDateTo = null;
        this.releaseFrom = null;
        this.releaseTo = null;
    ...
    search(): void {
        if (!this.selectedPresentStatus) {
            this.messageService.add({ severity: 'warn', summary: 'Report Type required', detail: 'Please select a report type (present status) first.' });
            return;
        }
        this.first = 0;
        this.searched = true;
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportPresentStatusComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    private loadPage(): void {
        ...
        // Type-specific date filters → DateRange criteria.
        const sFrom = this.fmtDate(this.statusDateFrom);
        const sTo = this.fmtDate(this.statusDateTo);
        if (sFrom || sTo) criteria.push({ fieldKey: 'statusDate', dateFrom: sFrom, dateTo: sTo });
        if (this.showReleaseDates) {
            const rFrom = this.fmtDate(this.releaseFrom);
            const rTo = this.fmtDate(this.releaseTo);
            if (rFrom || rTo) criteria.push({ fieldKey: 'dateOfRelease', dateFrom: rFrom, dateTo: rTo });
        }

        // Map column keys → registry FieldKeys; always include presentStatus & statusDate.
        const columns = Array.from(new Set([...this.backendColumnKeys(), 'presentStatus', 'statusDate']));

        this.reportService
            .runDynamicPresentStatusReport({
                columns,
                criteria,
                postingStatusFilter: '',
                presentStatusTypeFilter: this.selectedPresentStatus || '',
            ...
            .subscribe({
                ...
                error: (err) => {
                    console.error(err);
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || 'Failed to load present-status report.' });
                    this.loading = false;
                }
            });
    }
    ...
    get reportTitle(): string {
        if (this.lang === 'en') {
            if (this.selectedPresentStatus) return `${this.statusLabel(this.selectedPresentStatus)} Members Report`;
            return 'Employee Present-Status Report';
        }
        return 'কর্মচারীর বর্তমান অবস্থার প্রতিবেদন';
    }
    ...
    personnelMetaText(row: PresentStatusReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: PresentStatusReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
            ...
            case 'motherOrganization':
                return this.codeValue(row.motherOrganization, row.motherOrganizationBN);
            case 'presentUnit':
                return this.codeValue(row.presentUnit, row.presentUnitBN);
            case 'presentStatus':
                return this.statusLabel(row.presentStatus);
            case 'statusDate':
                return this.formatDateLabel(row.statusDate);
            case 'statusDetails':
    ...
    private extraCellValue(row: PresentStatusReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportPresentStatusComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportPresentStatusComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): PresentStatusReportRow {
        return {
            ...d,
            ser,
            ...
            motherOrganization: d['motherOrganization'] as string,
            motherOrganizationBN: d['motherOrganizationBN'] as string,
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `present-status-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `present-status-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: PresentStatusReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
