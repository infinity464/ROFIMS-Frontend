...
import type { RankWiseReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Rank-wise Report — one row per presently-serving employee, scoped to ONE
 * Mother Org and ONE Rank (both required). Optional Corps/Trade/RAB Unit/
 * Member Type refine the set. The backend applies the filters; the frontend
 * just displays. RAB-formal layout, dynamic column picker, drag-to-reorder,
 * Print/Word/Excel exports.
 */
@Component({
    selector: 'app-report-rank-wise',
    standalone: true,
...
export class ReportRankWiseComponent implements OnInit {
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: 'Serial' | 'Personnel' | 'Date' | 'Plain' | 'Remarks' | 'NameSuffix' | 'CallNoRankName'; defaultVisible: boolean }[] = [
        ...
        { key: 'ownHomeDistrict', labelEN: 'Own Home District', labelBN: 'নিজ জেলা', hint: 'Plain', defaultVisible: false },
    ...
    private static readonly colKeyToBackend: Record<string, string> = {
        ...
        joiningPresentUnit: 'joiningPresentUnit',
        presentWorkplace: 'presentWorkplace',
        rmks: 'rmks'
    };
    /** Extra (registry-keyed) columns rendered as formatted dates. */
    private static readonly extraDateKeys = new Set(['dob', 'joiningInRab', 'joiningPresentUnit', 'dateOfCommission']);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set(['rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'serviceId', 'postingStatus']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportRankWiseComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: RankWiseReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportRankWiseComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: RankWiseReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: RankWiseReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportRankWiseComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportRankWiseComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    get reportTitle(): string {
        return this.lang === 'en' ? 'Rank-wise Nominal Roll' : 'পদবি অনুযায়ী নামীয় তালিকা';
    }
    ...
    personnelMetaText(row: RankWiseReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: RankWiseReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
            ...
            case 'motherUnit':
                return this.codeValue(row.motherUnit, row.motherUnitBN);
            case 'dob':
                return this.formatDateLabel(row.dob);
            case 'joiningInRab':
                return this.formatDateLabel(row.joiningInRab);
            case 'joiningPresentUnit':
                return this.formatDateLabel(row.joiningPresentUnit);
            case 'presentWorkplace':
                return this.codeValue(row.presentWorkplace, row.presentWorkplaceBN);
            case 'rmks':
                return row.rmks ?? '';
            default:
    ...
    private extraCellValue(row: RankWiseReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportRankWiseComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportRankWiseComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): RankWiseReportRow {
        return {
            ...d,
            ser,
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `rank-wise-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `rank-wise-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: RankWiseReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
