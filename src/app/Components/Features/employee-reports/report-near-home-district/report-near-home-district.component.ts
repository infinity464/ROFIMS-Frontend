...
import type { NearHomeReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Near-Home-District Report (#10) — list of presently-serving members whose
 * serving unit's AOR district contains their own home district OR their wife's
 * present district. The backend applies the near-home filter; the frontend just
 * displays. RAB-formal layout, dynamic column picker, drag-to-reorder,
 * Print/Word/Excel exports.
 */
@Component({
    selector: 'app-report-near-home-district',
    standalone: true,
    ...
    templateUrl: './report-near-home-district.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-near-home-district.component.scss']
})
export class ReportNearHomeDistrictComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: 'Serial' | 'Personnel' | 'Date' | 'Plain' | 'Remarks' | 'NameSuffix' | 'CallNoRankName'; defaultVisible: boolean }[] = [
        ...
        { key: 'spouseHomeDistrict', labelEN: 'Spouse Home District', labelBN: "স্ত্রী'র জেলা", hint: 'Plain', defaultVisible: true },
    ...
    private static readonly colKeyToBackend: Record<string, string> = {
        ...
        currentUnitFrom: 'currentUnitFrom',
        ownHomeDistrict: 'ownHomeDistrict',
        spouseHomeDistrict: 'spouseHomeDistrict',
        rmks: 'rmks'
    };
    /** Extra (registry-keyed) columns rendered as formatted dates. */
    private static readonly extraDateKeys = new Set(['currentUnitFrom', 'dateOfCommission', 'rabServiceFrom', 'rabServiceTo', 'dob']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportNearHomeDistrictComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: NearHomeReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportNearHomeDistrictComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: NearHomeReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: NearHomeReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportNearHomeDistrictComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportNearHomeDistrictComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    private loadPage(): void {
        ...
        // Map column keys → registry FieldKeys; always include joiningInRab.
        const columns = Array.from(new Set([...this.backendColumnKeys(), 'joiningInRab']));

        this.reportService
            .runDynamicNearHomeReport({
                columns,
                criteria,
    ...
    get reportTitle(): string {
        return this.lang === 'en' ? 'Nominal Roll of Members Posted Near Home / Spouse District' : 'নিজ/স্ত্রীর জেলায় পদায়নকৃত সদস্যের নামীয় তালিকা';
    }
    ...
    personnelMetaText(row: NearHomeReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: NearHomeReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
            ...
            case 'currentUnitFrom':
                return this.formatDateLabel(row.currentUnitFrom);
            case 'presentUnit':
                return this.codeValue(row.presentUnit, row.presentUnitBN);
            case 'ownHomeDistrict':
                return this.codeValue(row.ownHomeDistrict, row.ownHomeDistrictBN);
            case 'spouseHomeDistrict':
                return this.codeValue(row.spouseHomeDistrict, row.spouseHomeDistrictBN);
            case 'rmks':
                return row.rmks ?? '';
            default:
    ...
    private extraCellValue(row: NearHomeReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportNearHomeDistrictComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportNearHomeDistrictComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): NearHomeReportRow {
        return {
            ...d,
            ser,
            ...
            currentUnitFrom: d['currentUnitFrom'] as string,
            ownHomeDistrict: d['ownHomeDistrict'] as string,
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `near-home-district-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `near-home-district-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: NearHomeReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
