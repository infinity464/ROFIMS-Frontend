 ...
 * Lists only members who have at least one confidential remark
 * (AdditionalRemarksInfo.IsConfidential = true). Optional remark text filter
 * narrows by Contains / LIKE.
 */
@Component({
    selector: 'app-report-confidential-remarks',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
...
export class ReportConfidentialRemarksComponent implements OnInit, OnDestroy {
    ...
    list: (MemberAppointmentReportRow & { confidentialRemarks?: string })[] = [];
    loading = false;
    first = 0;
    rows = 100;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'confidentialRemarks', labelEN: 'Confidential Remarks', labelBN: 'গোপনীয় মন্তব্য', hint: 'ConfidentialRemarks', defaultVisible: true },
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        ...
        rabUnit:             { en: 'presentUnit',       bn: 'presentUnitBN' },
        ...
        confidentialRemarks: { en: 'confidentialRemarks' },
        dob:                 { en: 'dob' },
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportConfidentialRemarksComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
        ...
        if (key === 'confidentialRemarks') return this.formatConfidentialRemarks(value);
        return value;
    }

    /** Each confidential remark on its own line (backend joins with \\n + leading •). */
    private formatConfidentialRemarks(value: string): string {
        if (!value || value === '-' || value === '—') return value;
        let lines = value.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
        if (lines.length === 1 && (lines[0].match(/•/g) || []).length > 1) {
            lines = lines[0].split(/\s*•\s*/).filter(Boolean).map((t) => `• ${t}`);
        }
        return lines.join('\n');
    }
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportConfidentialRemarksComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportConfidentialRemarksComponent.NAME_EXTRAS_KEY)) {
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.confidentialRemarks'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                switch (col.hint) {
                    ...
                    case 'ConfidentialRemarks': {
                        const text = this.plainCellValue(row, col.key);
                        const lines = text.split(/\r?\n/).filter((l) => l.length > 0);
                        const children = lines.length
                            ? lines.map((line, idx) => new Paragraph({
                                spacing: { after: idx < lines.length - 1 ? 40 : 0 },
                                children: [run(line)],
                            }))
        ...
        saveAs(blob, `confidential-remarks-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        for (let i = 0; i < this.list.length; i++) {
            ...
            const cells = visibleCols.map(col => {
                switch (col.hint) {
                    ...
                    case 'ConfidentialRemarks':   return this.plainCellValue(row, col.key);
                    case 'Plain':
        ...
        XLSX.writeFile(wb, `confidential-remarks-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: MemberAppointmentReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                ...
                case 'ConfidentialRemarks': {
                    const text = this.plainCellValue(row, col.key);
                    return `<td class="td-rmks">${esc(text).replace(/\n/g, '<br>')}</td>`;
                }
                case 'Plain':
                default:
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .td-rmks { white-space: pre-line; line-height: 1.35; }
...
<body>
    ...
    ngOnInit(): void {
        ...
        this.reportService.getMyReportAccessScope().subscribe({
            ...
            error: () => { /* leave dropdown editable on failure */ },
        });
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportConfidentialRemarksComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    get activeFilterCount(): number {
        let c = 0;
        if (this.remarksSearchText?.trim()) c++;
    ...
    load(): void {
        ...
        const search = (this.remarksSearchText ?? '').trim();

        this.reportService.getConfidentialRemarksReport({
            ...
            next: (res) => {
                ...
                this.list = (res.datalist ?? []).map((d, i) => this.adaptRow(d, startSer + i));
    ...
    private adaptRow(d: ConfidentialRemarksReportRow, ser: number): MemberAppointmentReportRow & { confidentialRemarks?: string } {
        return {
            ...(d as object),
            ser,
            ...
            confidentialRemarks: d['confidentialRemarks'] as string,
            ...(d['rabId'] ? { rabid: d['rabId'] as string } : {}),
        } as MemberAppointmentReportRow & { confidentialRemarks?: string };
    }
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        const raw = (row as Record<string, unknown>)['status'] as string | null | undefined;
        if (!raw) return '—';
        const m = ReportConfidentialRemarksComponent.MEMBER_STATUS_LABELS[raw];
