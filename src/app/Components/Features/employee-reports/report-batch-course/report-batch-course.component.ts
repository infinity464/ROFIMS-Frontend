...
import type { BatchCourseReportRow, ReportAccessibleScope } from '@/models/report.model';
...
export class ReportBatchCourseComponent implements OnInit, OnChanges {
    ...
    getExportData(): { columns: string[]; rows: string[][] } {
        ...
        const rows = this.list.map((row) => [
            ...
            this.codeValue(row.courseBatch, row.courseBatchBN),
            row.rmks ?? '—',
        ]);
        return { columns, rows };
    }
    ...
    load(): void {
        ...
        this.reportService
            .getBatchCourseReport({
                ...
                courseBatchId: this.commonCodeId ?? undefined,
