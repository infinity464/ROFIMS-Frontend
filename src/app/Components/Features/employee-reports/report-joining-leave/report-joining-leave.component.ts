...
import type { JoiningLeaveReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Joining-Leave Report (#15) — list of presently-serving members who are on
 * joining leave (moving from one Bn/Wg to another). The backend applies the
 * membership + on-leave filter; the frontend just displays. RAB-formal layout,
 * dynamic column picker, drag-to-reorder, Print/Word/Excel exports.
 */
@Component({
    selector: 'app-report-joining-leave',
    standalone: true,
    ...
    templateUrl: './report-joining-leave.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-joining-leave.component.scss']
})
export class ReportJoiningLeaveComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
    ...
    /** Joining-leave date-window filters. */
    joiningLeaveFrom: Date | null = null;
    joiningLeaveTo: Date | null = null;
    ...
    private static readonly colKeyToBackend: Record<string, string> = {
        ...
        corps: 'corps',
        trade: 'trade',
        presentBnWg: 'presentBnWg',
        postedBnWg: 'postedBnWg',
        joiningDate: 'joiningDate',
        leaveDuration: 'leaveDuration',
        rmks: 'rmks'
    };
    /** Extra (registry-keyed) columns rendered as formatted dates. */
    private static readonly extraDateKeys = new Set(['joiningDate', 'joiningLeaveFrom', 'joiningInRab', 'dateOfCommission', 'dob']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportJoiningLeaveComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: JoiningLeaveReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportJoiningLeaveComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: JoiningLeaveReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: JoiningLeaveReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportJoiningLeaveComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
    ...
    get activeFilterCount(): number {
        ...
        if (this.joiningLeaveFrom != null) c++;
        if (this.joiningLeaveTo != null) c++;
        return c;
    }
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        if (this.joiningLeaveFrom != null) items.push({ label: 'Joining Leave From', value: this.formatDateLabel(this.fmtDate(this.joiningLeaveFrom)) });
        if (this.joiningLeaveTo != null) items.push({ label: 'Joining Leave To', value: this.formatDateLabel(this.fmtDate(this.joiningLeaveTo)) });
        return items;
    }
    ...
    clearFilters(): void {
        ...
        this.joiningLeaveFrom = null;
        this.joiningLeaveTo = null;
        this.first = 0;
    }
    toggleLang(): void {
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportJoiningLeaveComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    private loadPage(): void {
        ...
        this.reportService
            .runDynamicJoiningLeaveReport({
                columns,
                criteria,
                idSearchText: (this.idSearchText ?? '').trim() || undefined,
                postingStatusFilter: 'Servings',
                joiningLeaveDateFrom: this.fmtDate(this.joiningLeaveFrom),
                joiningLeaveDateTo: this.fmtDate(this.joiningLeaveTo),
    ...
    get reportTitle(): string {
        return this.lang === 'en' ? 'List of Members on Joining Leave' : 'যোগদানকালীন ছুটিতে থাকা সদস্যের তালিকা';
    }
    ...
    personnelMetaText(row: JoiningLeaveReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: JoiningLeaveReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
            ...
            case 'presentBnWg':
                return this.codeValue(row.presentBnWg, row.presentBnWgBN);
            case 'postedBnWg':
                return this.codeValue(row.postedBnWg, row.postedBnWgBN);
            case 'joiningDate':
                return this.formatDateLabel(row.joiningDate);
            case 'leaveDuration':
                return this.formatLeaveDuration(row.leaveDuration);
            case 'rmks':
    ...
    /** Movement remarks are stored as rich-text HTML; render them as plain text
        (strip tags, decode common entities, collapse whitespace) so the table
        and exports don't show raw markup like "<p>…</p>". */
    ...
    private extraCellValue(row: JoiningLeaveReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportJoiningLeaveComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportJoiningLeaveComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): JoiningLeaveReportRow {
        return {
            ...d,
            ser,
            ...
            presentBnWg: d['presentBnWg'] as string,
            presentBnWgBN: d['presentBnWgBN'] as string,
            postedBnWg: d['postedBnWg'] as string,
            postedBnWgBN: d['postedBnWgBN'] as string,
            joiningDate: d['joiningDate'] as string,
            leaveDuration: (d['leaveDuration'] as number) ?? null,
            rmks: (d['rmks'] as string) ?? null
        } as JoiningLeaveReportRow;
    }
    ...
    private async exportRabWord(): Promise<void> {
                    ...
                    new TableCell({
                        ...
                        children: it
                            ? [
                                  ...
                                  // One paragraph per line — multi-line values (RAB
                                  // Unit org paths) keep their line breaks in Word.
                                  ...wsafe(it.value)
                                      .split('\n')
        ...
        saveAs(blob, `joining-leave-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `joining-leave-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: JoiningLeaveReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
