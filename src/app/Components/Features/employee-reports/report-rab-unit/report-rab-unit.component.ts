...
export class ReportRabUnitComponent implements OnInit, OnChanges, OnDestroy {
    L = REPORT_LABELS;
     ...
     * whose placement at the node's level is one of the picked ids.
     */
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportRabUnitComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportRabUnitComponent.NAME_EXTRAS_KEY)) {
    ...
    /** Column selection changed — re-fetch so newly added columns are populated. Only reloads once a search has been run. */
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportRabUnitComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
