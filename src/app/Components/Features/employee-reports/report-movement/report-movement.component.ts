...
import type { MovementReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
/**
 * Movement Report ("Members on Movement") — list of presently-serving members
 * who are on movement (one row per movement × member). Defaults to members not
 * yet returned (currently on movement). The backend explodes the movement's
 * JSON EmployeeIds and applies the membership + returned + release-date-window
 * filters; the frontend just displays. RAB-formal layout, dynamic column
 * picker, drag-to-reorder, Print/Word/Excel exports.
 */
@Component({
    selector: 'app-report-movement',
    standalone: true,
    ...
    templateUrl: './report-movement.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-movement.component.scss']
})
export class ReportMovementComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
     ...
     * (Unit / Wing / Branch / Sub-Branch / Section / Sub-Section). Selected node
     * ids are sent to the backend as the `rabOrgNode` criterion.
     */
    ...
    movementTypeOptions = [
        { label: 'Permanent', labelBn: 'স্থায়ী', value: 1 },
        { label: 'Temporary', labelBn: 'অস্থায়ী', value: 2 }
    ];
    moveOrderTypeOptions = [
        { label: 'CC', labelBn: 'সিসি', value: 1 },
        { label: 'MO', labelBn: 'এমও', value: 2 },
        { label: 'Article 47 (Handover)', labelBn: '৪৭ ধারা (হস্তান্তর)', value: 3 },
        { label: 'Article 47 (Takeover)', labelBn: '৪৭ ধারা (গ্রহণ)', value: 4 }
    ];
    returnedOptions = [
        { label: 'On Movement (Not Returned)', labelBn: 'মুভমেন্টে (প্রত্যাবর্তিত নয়)', value: 'NotReturned' },
        { label: 'Returned', labelBn: 'প্রত্যাবর্তিত', value: 'Returned' },
        { label: 'All', labelBn: 'সকল', value: 'All' }
    ];
    // Default to Temporary movements not yet returned ("on movement now").
    selectedMovementType: number | null = 2;
    selectedMoveOrderType: number | null = null;
    selectedReasonId: number | null = null;
    selectedDestinationId: number | null = null;
    selectedReturned: string = 'NotReturned';
    releaseFromDate: Date | null = null;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: 'Serial' | 'Personnel' | 'Date' | 'Plain' | 'Returned' | 'Remarks' | 'NameSuffix' | 'CallNoRankName'; defaultVisible: boolean }[] = [
        ...
        { key: 'isReturned', labelEN: 'Returned', labelBN: 'প্রত্যাবর্তিত', hint: 'Returned', defaultVisible: true },
        ...
        { key: 'dateOfReduce', labelEN: 'Reduce Date', labelBN: 'হ্রাসের তারিখ', hint: 'Date', defaultVisible: false },
    ...
    private static readonly colKeyToBackend: Record<string, string> = {
        ...
        destinedUnit: 'destinedUnit',
        dateOfRelease: 'dateOfRelease',
        isReturned: 'isReturned',
        rmks: 'rmks'
    };
    /** Extra (registry-keyed) columns rendered as formatted dates. */
    private static readonly extraDateKeys = new Set(['dateOfRelease', 'dateOfReduce', 'takeoverDate', 'handoverDate', 'returnDate', 'joiningInRab', 'dateOfCommission', 'dob']);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set(['rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'postingStatus', 'serviceId']);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportMovementComponent.NAME_EXTRAS_KEY)
            .map((k) => map.get(k))
     ...
     */
    nameColumnValue(row: MovementReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportMovementComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: MovementReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: MovementReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    /** Picker selection changed — the backend only returns the requested columns,
        so a reload is required to fetch any newly-added column's data. */
    ...
    loadOrgNodeLabels(): void {
        forkJoin(ReportMovementComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))).subscribe({
            next: (buckets) => {
     ...
     * are listed comma-separated (each keeping its sub-path below the unit).
     */
    ...
    private loadReasons(): void {
        this.commonCodeService.getAllActiveCommonCodesType('MovementReason').subscribe({
            next: (codes: CommonCodeModel[]) => (this.reasonOptions = this.mapCodes(codes)),
            error: () => (this.reasonOptions = [])
        });
    }
    ...
    /** Returned filter only applies to Temporary movements — Permanent moves
     *  don't have a return. Hidden (and not sent) when Permanent is selected. */
    get showReturnedFilter(): boolean {
        return this.selectedMovementType !== 1;
    }
    ...
    /** Format a Date as 'yyyy-MM-dd' (or null). */
    ...
    get activeFilterCount(): number {
        ...
        if (this.selectedMovementType != null && this.selectedMovementType !== 2) c++;
        if (this.selectedMoveOrderType != null) c++;
        if (this.selectedReasonId != null) c++;
        if (this.selectedDestinationId != null) c++;
        if (this.showReturnedFilter && this.selectedReturned && this.selectedReturned !== 'NotReturned') c++;
        if (this.releaseFromDate != null) c++;
        if (this.releaseToDate != null) c++;
        return c;
    }
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        const single = (id: number | string | null, opts: { label: string; labelBn: string; value: any }[], en: string, bn: string) => {
            if (id == null) return;
            const opt = opts.find((o) => o.value === id);
            if (opt) items.push({ label: this.lang === 'en' ? en : bn, value: this.lang === 'bn' ? opt.labelBn : opt.label });
        };
        ...
        single(this.selectedReasonId, this.reasonOptions, 'Reason', 'কারণ');
        single(this.selectedDestinationId, this.rabUnitOptions, 'Destination', 'গন্তব্য ইউনিট');
        if (this.showReturnedFilter && this.selectedReturned && this.selectedReturned !== 'NotReturned') single(this.selectedReturned, this.returnedOptions, 'Returned', 'প্রত্যাবর্তিত');
        if (this.releaseFromDate != null) items.push({ label: this.lang === 'en' ? 'Release From' : 'অবমুক্তি (শুরু)', value: this.formatDateLabel(this.fmtDate(this.releaseFromDate)) });
        if (this.releaseToDate != null) items.push({ label: this.lang === 'en' ? 'Release To' : 'অবমুক্তি (শেষ)', value: this.formatDateLabel(this.fmtDate(this.releaseToDate)) });
        return items;
    }
    ...
    clearFilters(): void {
        ...
        this.selectedMovementType = 2;
        this.selectedMoveOrderType = null;
        this.selectedReasonId = null;
        this.selectedDestinationId = null;
        this.selectedReturned = 'NotReturned';
        this.releaseFromDate = null;
        this.releaseToDate = null;
        this.idSearchText = '';
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportMovementComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    private loadPage(): void {
        ...
        this.reportService
            .runDynamicMovementReport({
                columns,
                criteria,
                postingStatusFilter: 'Servings',
                movementType: this.selectedMovementType,
                moveOrderType: this.selectedMoveOrderType,
                movementReasonId: this.selectedReasonId,
                destinedRabUnitId: this.selectedDestinationId,
                movementReturnedFilter: this.showReturnedFilter ? this.selectedReturned : 'All',
                movementDateFrom: this.fmtDate(this.releaseFromDate),
                movementDateTo: this.fmtDate(this.releaseToDate),
            ...
            .subscribe({
                ...
                error: (err) => {
                    console.error(err);
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || 'Failed to load movement list.' });
                    this.loading = false;
                }
            });
    }
    ...
    get reportTitle(): string {
        return this.lang === 'en' ? 'List of Members on Movement' : 'মুভমেন্টে থাকা সদস্যের তালিকা';
    }
    ...
    personnelMetaText(row: MovementReportRow): string {
        const bits: string[] = [];
    ...
    cellValue(row: MovementReportRow, key: string): string {
        switch (key) {
            case 'serviceId': {
            ...
            case 'corps':
                return this.codeValue(row.corps, row.corpsBN);
            case 'trade':
            ...
            case 'movementType':
                return this.codeValue(row.movementType, row.movementTypeBN);
            case 'moveOrderType':
                return this.codeValue(row.moveOrderType, row.moveOrderTypeBN);
            case 'movementReason':
                return this.codeValue(row.movementReason, row.movementReasonBN);
            case 'currentUnit':
                return this.codeValue(row.currentUnit, row.currentUnitBN);
            case 'destinedUnit':
                return this.codeValue(row.destinedUnit, row.destinedUnitBN);
            case 'dateOfRelease':
                return this.formatDateLabel(row.dateOfRelease);
            case 'isReturned':
                return this.formatReturned(row.isReturned);
            case 'rmks':
    ...
    /** Boolean returned flag → "Yes"/"No" (or Bangla). */
    private formatReturned(v: boolean | null | undefined): string {
        if (v == null) return '—';
        if (this.lang === 'bn') return v ? 'হ্যাঁ' : 'না';
        return v ? 'Yes' : 'No';
    }

    /** Movement remarks are stored as rich-text HTML; render them as plain text. */
    ...
    private extraCellValue(row: MovementReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportMovementComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportMovementComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): MovementReportRow {
        return {
            ...d,
            ser,
            ...
            movementTypeBN: d['movementTypeBN'] as string,
            moveOrderType: d['moveOrderType'] as string,
            moveOrderTypeBN: d['moveOrderTypeBN'] as string,
            movementReason: d['movementReason'] as string,
            movementReasonBN: d['movementReasonBN'] as string,
            currentUnit: d['currentUnit'] as string,
            currentUnitBN: d['currentUnitBN'] as string,
            destinedUnit: d['destinedUnit'] as string,
            destinedUnitBN: d['destinedUnitBN'] as string,
            dateOfRelease: d['dateOfRelease'] as string,
            isReturned: (d['isReturned'] as boolean) ?? null,
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map((col) => {
                ...
                switch (col.hint) {
                    ...
                    case 'Remarks':
                        return new TableCell({ ...cellOpts, children: [new Paragraph({ children: [run(this.cellValue(row, 'rmks'), { color: C.gray })] })] });
                    case 'Returned':
                    case 'Plain':
                    default:
        ...
        saveAs(blob, `movement-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `movement-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: MovementReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
                ...
                case 'Remarks':
                    return `<td class="td-rmks">${esc(this.cellValue(row, 'rmks'))}</td>`;
                case 'Returned':
                case 'Plain':
                default:
