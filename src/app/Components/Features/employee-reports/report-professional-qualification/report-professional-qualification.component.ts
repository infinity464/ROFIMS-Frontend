...
/**
 * Professional Qualification Report — parent's `commonCodeId` is a
 * CommonCode of type "ProfessionalQualification". Drives the
 * `professionalQualification` criterion against
 * EmployeeInfo.ProfessionalQualification.
 */
@Component({
    selector: 'app-report-professional-qualification',
    standalone: true,
    ...
    templateUrl: './report-professional-qualification.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-professional-qualification.component.scss'],
})
export class ReportProfessionalQualificationComponent implements OnInit, OnChanges, OnDestroy {
    L = REPORT_LABELS;
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportProfessionalQualificationComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter(k => k !== ReportProfessionalQualificationComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportProfessionalQualificationComponent.NAME_EXTRAS_KEY)) {
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportProfessionalQualificationComponent.MEMBER_STATUS_LABELS[raw];
    ...
    get rabSectionTitle(): string {
        ...
        return this.L[this.lang]['report.title.professionalQualification'];
    }
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.professionalQualification'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `professional-qualification-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `professional-qualification-report_${this.lang}.xlsx`);
    }
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportProfessionalQualificationComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    load(): void {
        ...
        if (this.commonCodeId != null && this.commonCodeId > 0)
            criteria.push({ fieldKey: 'professionalQualification', idValue: this.commonCodeId });
