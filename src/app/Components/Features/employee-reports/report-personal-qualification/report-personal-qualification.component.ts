 ...
 * of type "PersonalQualification". Drives the `personalQualification`
 * criterion against EmployeeInfo.PersonalQualification.
 */
@Component({
    selector: 'app-report-personal-qualification',
    standalone: true,
...
export class ReportPersonalQualificationComponent implements OnInit, OnChanges {
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'personalQualification', labelEN: 'Personal Qualification', labelBN: 'ব্যক্তিগত যোগ্যতা', hint: 'Plain', defaultVisible: false },
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportPersonalQualificationComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportPersonalQualificationComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportPersonalQualificationComponent.NAME_EXTRAS_KEY)) {
    ...
    get rabSectionTitle(): string {
        ...
        return this.L[this.lang]['report.title.personalQualification'];
    }
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.personalQualification'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                switch (col.hint) {
                    ...
                    case 'BlankRemark': return new TableCell({ ...cellOpts, children: [new Paragraph({ children: [run('')] })] });
                    case 'Plain':
        ...
        saveAs(blob, `personal-qualification-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `personal-qualification-report_${this.lang}.xlsx`);
    }
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportPersonalQualificationComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportPersonalQualificationComponent.MEMBER_STATUS_LABELS[raw];
    ...
    load(): void {
        ...
        if (this.commonCodeId != null && this.commonCodeId > 0)
            criteria.push({ fieldKey: 'personalQualification', idValue: this.commonCodeId });
