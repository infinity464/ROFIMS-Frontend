...
/**
 * Unit Duration Nominal Roll Report — RAB-formal layout, dynamic column
 * picker, drag-to-reorder, Print/Word/Excel exports. RAB Unit is required;
 * Mother Org / Rank / date range are optional. Access control runs
 * server-side via the existing /GetUnitDurationNominalRollReport endpoint.
 */
@Component({
...
export class ReportUnitDurationNominalRollComponent implements OnInit, OnDestroy {
    ...
    private static readonly extraDateKeys = new Set([
        'dateOfCommission', 'joiningInRab', 'dob', 'relieverJoiningDate', 'possibleJoiningDate',
    ]);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set([
        'rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'postingStatus', 'relieverServiceId',
    ]);
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter(k => k !== ReportUnitDurationNominalRollComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
     ...
     */
    nameColumnValue(row: UnitDurationNominalRollReportRow): string {
        const r = row as any;
        const blank = (s: string | null | undefined) => !s || s === '-' || s === '—';
        const parts: string[] = [];
        const name = this.codeValue(r.name, r.nameBN);
        if (!blank(name)) parts.push(name);
        if (this.selectedColumnKeys.includes(ReportUnitDurationNominalRollComponent.NAME_EXTRAS_KEY)) {
    ...
    callNoRankLine1(row: UnitDurationNominalRollReportRow): string {
        const r = row as any;
    ...
    callNoRankLine2(row: UnitDurationNominalRollReportRow): string {
        const r = row as any;
        const name = this.codeValue(r.name, r.nameBN);
    ...
    get criteriaItems(): { label: string; value: string }[] {
        ...
        if (this.fromDate) {
            const lbl = this.lang === 'en' ? 'Duration From' : 'সময়কাল হইতে';
            items.push({ label: lbl, value: this.formatDateLabel(this.fmtDate(this.fromDate)!) });
        }
        if (this.toDate) {
            const lbl = this.lang === 'en' ? 'Duration To' : 'সময়কাল পর্যন্ত';
    ...
    search(): void {
        if (!this.selectedOrgNodeIds.length) {
            this.messageService.add({
                severity: 'warn',
                summary: this.lang === 'en' ? 'SRB Unit required' : 'এসআরবি ইউনিট প্রয়োজন',
                detail: this.lang === 'en' ? 'Please select at least one SRB Unit to generate this nominal roll.' : 'নামীয় তালিকা তৈরি করতে অন্তত একটি এসআরবি ইউনিট নির্বাচন করুন।',
            });
            return;
        }
        this.first = 0;
    ...
    private backendColumnKeys(): string[] {
        ...
        for (const key of this.selectedColumnKeys) {
            ...
            push(ReportUnitDurationNominalRollComponent.colKeyToBackend[key] ?? key);
        }
        return out;
    }
    ...
    get reportTitle(): string {
        return this.lang === 'en'
            ...
            : 'ইউনিট এবং এসআরবিে নির্দিষ্ট অবস্থান ভিত্তিক নামীয় তালিকা';
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `unit-duration-nominal-roll_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `unit-duration-nominal-roll_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: UnitDurationNominalRollReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
