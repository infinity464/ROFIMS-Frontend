...
import { memberTypeScopeLine } from '../report-scope.helper';
 ...
 * (PostingStatus IN {Supernumerary, Pending, PendingForJoining}).
 *
 * Access control: ONLY the member-type filter applies — supernumeraries have
 * no RAB posting yet, so the org-tree scope can't be checked against
 * PreviousRABServiceInfo. The dynamic backend's RunEmployeeBaseAsync skips
 * the org-tree filter automatically when `postingStatusFilter` is
 * "Supernumerary".
 *
 * Filters: Mother Org, Member Type, Rank, Trade. RAB-formal paper layout,
 * dynamic column picker + drag-reorder, Print/Word/Excel exports.
 */
@Component({
...
export class ReportSupernumeraryComponent implements OnInit {
    ...
    /** No unit-scope line — supernumeraries aren't org-placed, so the chip
        would be misleading. Only the member-type chip applies. */
    get unitScopeLine(): string | null { return null; }
    get memberTypeScopeLine(): string | null { return memberTypeScopeLine(this.accessibleScope, this.lang); }
    get statusLocked(): boolean { return false; }
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        ...
        postingStatus:       { en: 'postingStatus' },
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportSupernumeraryComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
        const en = (row as any)[map.en] as string | null | undefined;
        const bn = map.bn ? (row as any)[map.bn] as string | null | undefined : undefined;
        // Plain dates pass through unformatted from the backend (ISO yyyy-mm-dd).
        // For the curated rabServiceFrom column we want dd-mm-yyyy; do that here
        // so the user can still re-pick it via the column dropdown.
        if (key === 'rabServiceFrom' || key === 'joiningDate' || key === 'dateOfCommission' || key === 'rabServiceTo' || key === 'dob') {
            return this.formatDate(en ?? null);
        }
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter((k) => k !== ReportSupernumeraryComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportSupernumeraryComponent.NAME_EXTRAS_KEY)) {
    ...
    get rabSectionTitle(): string { return this.L[this.lang]['report.title.supernumerary']; }
     ...
     * "name" column maps to the registry's nameEnglish/nameBangla fields; the
     * nameExtras / callNoRankName composites request the extra fields they
     * render. ser is synthetic and ignored server-side.
     */
    ...
    load(): void {
        ...
        // postingStatusFilter = "Supernumerary" hits the special-case path in
        // RunEmployeeBaseAsync that (a) widens to Supernumerary + Pending +
        // PendingForJoining, (b) keeps the member-type access filter, and
        ...
        this.reportService.runDynamicEmployeeBaseReport({
            ...
            next: (res) => {
                ...
                // Preserve only the member-type chip — the org-tree chip would
                // be misleading here. We re-fetch the scope via /GetMyReportAccessScope
                // on init to capture both sets of access names; this response
                // is only kept for parity.
                this.searched = true;
                this.loading = false;
            },
            error: (err) => {
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `supernumerary-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `supernumerary-report_${this.lang}.xlsx`);
    }
