...
/**
 * Special Qualification Report — parent's `commonCodeId` is a CommonCode
 * of type "SpecialQualification". EmployeeInfo.SpecialQualifications is a
 * CSV ("12,45,78"), so we filter via the `specialQualificationMatch`
 * sentinel which does a padded LIKE membership check server-side.
 *
 * No column for the matched value — it would surface the entire CSV, not
 * the specific qualification the user picked. The criteria strip / report
 * title already display the picked qualification name.
 */
@Component({
    selector: 'app-report-special-qualification',
    standalone: true,
    ...
    templateUrl: './report-special-qualification.component.html',
    styleUrls: ['../report-theme.scss', '../report-card-mtr.scss', './report-special-qualification.component.scss'],
})
export class ReportSpecialQualificationComponent implements OnInit, OnChanges, OnDestroy {
    L = REPORT_LABELS;
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        ...
        personalQualification:     { en: 'personalQualification',     bn: 'personalQualificationBN' },
        professionalQualification: { en: 'professionalQualification', bn: 'professionalQualificationBN' },
        dob:                 { en: 'dob' },
    ...
    plainCellValue(row: MemberAppointmentReportRow, key: string): string {
        ...
        const map = ReportSpecialQualificationComponent.plainColumnPropertyMap[key];
        if (!map) return '—';
    ...
    nameColumnValue(row: MemberAppointmentReportRow): string {
        ...
        if (this.selectedColumnKeys.includes(ReportSpecialQualificationComponent.NAME_EXTRAS_KEY)) {
    ...
    get visibleColumns(): typeof this.columnCatalog {
        ...
        return this.selectedColumnKeys
            .filter(k => k !== ReportSpecialQualificationComponent.NAME_EXTRAS_KEY)
            .map(k => map.get(k))
    ...
    memberStatusText(row: MemberAppointmentReportRow): string {
        ...
        if (!raw) return '—';
        const m = ReportSpecialQualificationComponent.MEMBER_STATUS_LABELS[raw];
    ...
    get rabSectionTitle(): string {
        ...
        return this.L[this.lang]['report.title.specialQualification'];
    }
    ...
    get reportTitle(): string {
        ...
        return this.L[this.lang]['report.title.specialQualification'] + statusSuffix;
    }
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `special-qualification-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `special-qualification-report_${this.lang}.xlsx`);
    }
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    loadOrgNodeLabels(): void {
        forkJoin(
            ReportSpecialQualificationComponent.ORG_CODE_TYPES.map((t) => this.commonCodeService.getAllActiveCommonCodesType(t))
        ).subscribe({
            next: (buckets) => {
    ...
    load(): void {
        ...
        if (this.commonCodeId != null && this.commonCodeId > 0)
            criteria.push({ fieldKey: 'specialQualificationMatch', idValue: this.commonCodeId });
