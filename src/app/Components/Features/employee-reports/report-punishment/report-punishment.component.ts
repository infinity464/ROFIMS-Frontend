...
import type { PunishmentReportRow, ReportAccessibleScope, DynamicReportCriterion, DynamicReportRow } from '@/models/report.model';
...
import { debounceTime, forkJoin, of, Subject, Subscription } from 'rxjs';
import { catchError } from 'rxjs/operators';

type Lang = 'en' | 'bn';

/**
 * Punishment Report (#23) — one row per (employee × discipline record). The
 * backend applies the membership + punishment filters; the frontend just
...
export class ReportPunishmentComponent implements OnInit, OnDestroy {
    lang: Lang = 'en';
    ...
    /** Punishment-date-window filters (RAB + Mother-Org date ranges). */
    punRabFrom: Date | null = null;
    ...
    private static readonly extraDateKeys = new Set(['punishmentDateMo', 'offenseDate', 'dob']);
    /** Extra columns that are plain identifiers/text (no BN mirror). */
    private static readonly extraPlainKeys = new Set(['rabId', 'nameBangla', 'nid', 'bloodGroup', 'mobileNo', 'email', 'postingStatus', 'serviceId', 'offenceDetails', 'disciplineAction']);
    ...
    private loadPage(): void {
        ...
        this.reportService
            ...
            .subscribe({
                ...
                error: (err) => {
                    console.error(err);
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: err?.error?.message || 'Failed to load punishment report.' });
                    this.loading = false;
                }
            });
    }
    ...
    personnelMetaText(row: PunishmentReportRow): string {
        const bits: string[] = [];
    ...
    /** Rich-text remarks/offence details are stored as HTML; render them as plain
        text (strip tags, decode common entities, collapse whitespace) so the table
    ...
    private extraCellValue(row: PunishmentReportRow, key: string): string {
        const anyRow = row as any;
        if (ReportPunishmentComponent.extraDateKeys.has(key)) return this.formatDateLabel(anyRow[key]);
        if (ReportPunishmentComponent.extraPlainKeys.has(key)) return this.displayNum(anyRow[key]);
        return this.codeValue(anyRow[key], anyRow[key + 'BN']);
    }

    /** Translate a dynamic-backend property bag into the legacy row shape. */
    private adaptDynamicRow(d: DynamicReportRow, ser: number): PunishmentReportRow {
        return {
            ...d,
            ser,
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `punishment-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `punishment-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: PunishmentReportRow, col: { key: string; hint: string }, idx: number): string => {
            switch (col.hint) {
                case 'Serial':
