...
@Component({
  ...
  templateUrl: './my-login-audit.component.html'
})
export class MyLoginAuditComponent implements OnInit {
  audits: LoginAudit[] = [];
  totalRecords = 0;
  loading = false;
  rows = 10;
  ...
  loadData(): void {
    ...
    this.auditService
      .getByUserId(this.userId, page, this.rows, this.selectedStatus || undefined, fromStr, toStr)
      .subscribe({
        next: (res) => {
