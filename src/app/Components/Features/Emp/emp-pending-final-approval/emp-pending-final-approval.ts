...
export class EmpPendingFinalApprovalComponent implements OnInit {
    ...
    // ---------- Nominal roll export ----------
    /** Id of the list whose export is currently being fetched. */
    exportingId: number | null = null;

    /**
     * Fetches the roll then hands it to the chosen renderer. These are still
     * DraftCourseList rows awaiting final approval, so they reuse the draft
     * nominal-roll endpoint. Read fresh each time rather than reusing the
     * panel's members, because the export needs the Bangla names and units the
     * panel never loads.
     *
     * annexureLabel is null — a list pending approval is not an annexure, so the
     * roll drops the "ক্রোড়পত্র ক" line.
     */
