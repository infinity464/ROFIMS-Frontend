...
/**
 * Equivalent-name variant of the rank-wise manpower report. Rows are EquivalentName
 * (the equivalent-name man-power setup); Auth comes from EquivalentNameVacancyDistribution,
 * Held / Posted Out are mapped via RankEquivalent. Reuses the rank-wise SCSS; the rank
 * axis here is already an equivalent name so the "show equivalent ranks" toggle is dropped.
 */
@Component({
    selector: 'app-equivalent-name-wise-manpower',
    standalone: true,
    imports: [CommonModule, FormsModule, MultiSelectModule, CheckboxModule, OrgTreeFilterComponent],
    templateUrl: './equivalent-name-wise-manpower.html',
    styleUrl: '../rank-wise-manpower/rank-wise-manpower.scss'
})
export class EquivalentNameWiseManpowerComponent implements OnInit {
    canInsert = true;
    canUpdate = true;
    canDelete = true;
    ...
    loadData(): void {
        this.loading = true;
        this.statisticsService.getEquivalentNameWiseManpower(this.filterRabCodeId).subscribe({
    ...
    async exportAs(type: 'pdf' | 'print' | 'word' | 'excel'): Promise<void> {
        ...
        const sectionedConfig = {
            ...
            filename: 'equivalent-name-wise-manpower',
    ...
    get titleLabel(): string {
        return this.lang === 'en'
            ? 'ORGANIZATION & SRB RANK WISE MANPOWER STATE'
            : 'বাহিনী এবং এসআরবি পদবী অনুযায়ী জনবলের সারাংশ';
    }
    ...
    get dateLine(): string {
        ...
        if (this.lang === 'en') {
            return `${day} ${EquivalentNameWiseManpowerComponent.EN_MONTHS[mon]} ${year}`;
        }
        return `${BanglaNumerals.toBangla(String(day))} ${EquivalentNameWiseManpowerComponent.BN_MONTHS[mon]} ${BanglaNumerals.toBangla(String(year))}`;
    }

    get colHeaders(): string[] {
        const base = this.lang === 'en'
            ? ['Ser', 'Equivalent Name', 'Auth', 'Held', 'Def', 'Sur', 'Def %']
            : ['ক্রমিক', 'সমতুল্য নাম', 'প্রাধিকার', 'বিদ্যমান', 'ঘাটতি', 'অতিরিক্ত', 'ঘাটতি %'];
