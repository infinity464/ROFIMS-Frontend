...
import {
    StatisticsService,
    type UnitSpecialQualificationColumn,
    type UnitSpecialQualificationRow,
    type UnitSpecialQualificationWiseManpowerResponse
} from '@/services/statistics.service';
import { RabReportPrintService } from '../shared/rab-report-print.service';

interface UnitSpecialQualSection {
    titleEN: string;
    titleBN: string;
...
@Component({
    selector: 'app-unit-special-qualification-wise-manpower',
    standalone: true,
    imports: [CommonModule, FormsModule, MultiSelectModule],
    templateUrl: './unit-special-qualification-wise-manpower.html',
    styleUrl: '../unit-rank-wise-manpower/unit-rank-wise-manpower.scss'
})
export class UnitSpecialQualificationWiseManpowerComponent implements OnInit {
    canInsert = true;
    canUpdate = true;
    canDelete = true;
    ...
    sections: UnitSpecialQualSection[] = [];
    ...
    selectedQualColIds: number[] = [];

    get qualVisOptions(): { label: string; value: number }[] {
        return this.qualifications.map(q => ({ label: this.qualLabel(q), value: q.qualificationColId }));
    }

    private resetQualVisibility(): void {
        this.selectedQualColIds = this.qualifications.map(q => q.qualificationColId);
    }

    visibleQualCols(): UnitSpecialQualificationColumn[] {
        return this.qualifications.filter(q => this.selectedQualColIds.includes(q.qualificationColId));
    }

    /** Distinct members with at least one special qualification (from API). */
    rowTotalVisible(row: UnitSpecialQualificationRow): number {
        return row.total ?? 0;
    }

    sectionGrandTotalVisible(section: UnitSpecialQualSection): number {
        return section.units.reduce((s, u) => s + this.rowTotalVisible(u), 0);
    }
    ...
    loadData(): void {
        ...
        if (ids.length === 0) {
            this.statisticsService.getUnitSpecialQualificationWiseManpower(null).subscribe({
                next: (res: UnitSpecialQualificationWiseManpowerResponse) => {
        ...
        forkJoin(
            ids.map(id =>
                this.statisticsService.getUnitSpecialQualificationWiseManpower(id)
                    .pipe(catchError(() => of(null as UnitSpecialQualificationWiseManpowerResponse | null)))
            )
        ).subscribe({
            next: (results) => {
                const built: UnitSpecialQualSection[] = [];
                let firstQuals: UnitSpecialQualificationColumn[] = [];
                let scopeEN: string[] | null = null;
                let scopeBN: string[] | null = null;
                results.forEach((res, i) => {
                    if (!res) return;
                    if (firstQuals.length === 0) firstQuals = res.qualifications ?? [];
                ...
                this.qualifications = firstQuals;
                this.sections = built;
    ...
    cell(row: UnitSpecialQualificationRow, colId: number): number {
        return row.cells?.[colId] ?? 0;
    }

    colTotal(section: UnitSpecialQualSection, colId: number): number {
        return section.columnTotals?.[colId] ?? 0;
    }

    sectionTitle(section: UnitSpecialQualSection): string {
    ...
    unitLabel(row: UnitSpecialQualificationRow): string {
        return this.lang === 'en' ? row.unitNameEN : (row.unitNameBN || row.unitNameEN);
    }

    qualLabel(col: UnitSpecialQualificationColumn): string {
        return this.lang === 'en' ? col.qualificationNameEN : (col.qualificationNameBN || col.qualificationNameEN);
    }
    ...
    get titleLabel(): string {
        ...
        return this.lang === 'en'
            ? `${rowEN}-WISE MANPOWER STATE BY SPECIAL QUALIFICATION`
            : `${rowBN} অনুযায়ী বিশেষ যোগ্যতা ভিত্তিক জনবলের পরিসংখ্যান`;
    }
    ...
    get dateLine(): string {
        ...
        if (this.lang === 'en') {
            return `${day} ${UnitSpecialQualificationWiseManpowerComponent.EN_MONTHS[mon]} ${year}`;
        }
        const dayBN  = BanglaNumerals.toBangla(String(day));
        const yearBN = BanglaNumerals.toBangla(String(year));
        return `${dayBN} ${UnitSpecialQualificationWiseManpowerComponent.BN_MONTHS[mon]} ${yearBN}`;
    }
    ...
    async exportAs(type: 'pdf' | 'print' | 'word' | 'excel'): Promise<void> {
        ...
        const visibleCols = this.visibleQualCols();
        const columns = [this.unitHeader, ...visibleCols.map(q => this.qualLabel(q)), this.totalHeader];
        const sectionedConfig = {
            ...
            sections: this.sections.map(sec => ({
                ...
                rows: sec.units.map(u => [
                    this.unitLabel(u),
                    ...visibleCols.map(q => this.fmt(this.cell(u, q.qualificationColId))),
                    this.fmt(this.rowTotalVisible(u))
                ]),
                subtotalRow: [
                    this.totalHeader,
                    ...visibleCols.map(q => this.fmt(this.colTotal(sec, q.qualificationColId))),
                    this.fmt(this.sectionGrandTotalVisible(sec))
                ]
            })),
            showPageNumbers: true,
            filename: 'unit-special-qualification-wise-manpower',
            ...
            this.rabPrint.print({
                ...
                sections: this.sections.map(sec => ({
                    ...
                    columns: [
                        ...
                        ...visibleCols.map(q => ({ label: this.qualLabel(q), align: 'center' as const, mono: true })),
