...
/**
 * Standalone Nominee Information report — one row per nominee for the
 * searched member. Nominees are family members (joined by FMID) with a
 * nominated share percentage. Mirrors the structure of report-address /
 * report-education under individual-reports.
 */
type NomineeReportRow = {
    ser?: number;
    fmid: number;
    nominee: string;
    nomineeBN: string;
...
@Component({
    selector: 'app-report-nominee-individual',
    standalone: true,
    ...
    templateUrl: './report-nominee.component.html',
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-nominee.component.scss'],
})
export class ReportNomineeIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'nominee',       labelEN: 'Nominee Name',   labelBN: 'নমিনির নাম',     hint: 'Plain',     defaultVisible: true  },
        { key: 'relation',      labelEN: 'Relation',       labelBN: 'সম্পর্ক',        hint: 'Plain',     defaultVisible: true  },
        { key: 'sharePercent',  labelEN: 'Share (%)',      labelBN: 'অংশ (%)',        hint: 'Percent',   defaultVisible: true  },
        { key: 'nominatedDate', labelEN: 'Nominated Date', labelBN: 'মনোনয়নের তারিখ', hint: 'PlainDate', defaultVisible: false },
    ];

    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        nominee:       { en: 'nominee',       bn: 'nomineeBN' },
        relation:      { en: 'relation',      bn: 'relationBN' },
        nominatedDate: { en: 'nominatedDate' },
    };

    plainCellValue(row: NomineeReportRow, key: string): string {
        const map = ReportNomineeIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    plainDateCellValue(row: NomineeReportRow, key: string): string {
        const map = ReportNomineeIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
        return this.formatDate((row as any)[map.en] as string | null | undefined);
    }

    percentCell(row: NomineeReportRow): string {
        return row.sharePercent == null ? '-' : `${this.displayNum(row.sharePercent)}%`;
    }
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের কোনো নমিনি তথ্য নেই।' : 'No nominee records found for this member.';
    }
    ...
    get rabSectionTitle(): string { return this.lang === 'bn' ? 'নমিনি তথ্য প্রতিবেদন' : 'NOMINEE INFORMATION REPORT'; }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        forkJoin({
            nominees: this.nomineeInfoService.getByEmployeeId(employeeId).pipe(catchError(() => of([] as any[]))),
            family: this.familyInfoService.getByEmployeeId(employeeId).pipe(catchError(() => of([] as any[]))),
            relations: this.commonCodeService.getAllActiveCommonCodesType('Relationship').pipe(catchError(() => of([] as any[]))),
        }).subscribe({
            next: ({ nominees, family, relations }) => {
                const relMap = this.buildCodeMap(relations);
                const familyMap = this.buildFamilyMap(family ?? [], relMap);
                const shaped = (nominees ?? []).map((n: any) => {
                    const fmid = Number(n.fmid ?? n.FMID ?? 0);
                    const fm = familyMap.get(fmid);
                    const pctRaw = n.sharePercent ?? n.SharePercent ?? null;
                    const date = n.createdDate ?? n.CreatedDate ?? n.lastupdate ?? n.Lastupdate ?? null;
                    return {
                        fmid,
                        nominee: fm?.en || `Family Member #${fmid}`,
                        nomineeBN: fm?.bn || (fm?.en || `Family Member #${fmid}`),
                        relation: fm?.relEn ?? null,
                        relationBN: fm?.relBn ?? null,
                        sharePercent: pctRaw != null ? Number(pctRaw) : null,
                        nominatedDate: date != null ? (typeof date === 'string' ? date : new Date(date).toISOString()) : null,
                    } as NomineeReportRow;
                });
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load nominee information' });
                this.loading = false;
            },
        });
    }
    ...
    private buildFamilyMap(family: any[], relMap: Map<number, CodeName>): Map<number, { en: string | null; bn: string | null; relEn: string | null; relBn: string | null }> {
        const map = new Map<number, { en: string | null; bn: string | null; relEn: string | null; relBn: string | null }>();
        for (const m of family ?? []) {
            const fmid = m.FMID ?? m.fmid;
            if (fmid == null) continue;
            const relId = m.Relation ?? m.relation;
            const rel = relId != null ? relMap.get(Number(relId)) : undefined;
            map.set(Number(fmid), {
    ...
    private cellText(row: NomineeReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
            case 'Serial':    return this.paddedSer(row.ser);
            case 'Percent':   return this.percentCell(row);
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportNomineeIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                const isMono = isSerial || col.hint === 'PlainDate';
        ...
        saveAs(blob, `nominee-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `nominee-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: NomineeReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
                case 'Serial':    return `<td class="td-ser"><span class="ser">${esc(this.paddedSer(row.ser))}</span></td>`;
                case 'PlainDate': return `<td class="td-date">${esc(this.plainDateCellValue(row, col.key))}</td>`;
