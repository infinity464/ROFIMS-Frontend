...
/**
 * Standalone "Discipline Issue" report — one row per discipline record for
 * the searched member. Source view (vw_DisciplineInfoByEmployee) already
 * resolves offence/punishment type names.
 */
type DisciplineReportRow = {
    ser?: number;
    offenseDate: string | null;
    offenseType: string | null; offenseTypeBN: string | null;
    briefStatement: string | null; briefStatementBN: string | null;
    offenseDetails: string | null;
    punishmentRAB: string | null; punishmentRABBN: string | null;
    punishmentDate: string | null;
    punishmentMO: string | null; punishmentMOBN: string | null;
...
export class ReportDisciplineIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'punishmentMO',     labelEN: 'Punishment (Org)', labelBN: 'শাস্তি (সংস্থা)',   hint: 'Plain',     defaultVisible: false },
        { key: 'punishmentDateMO', labelEN: 'Punishment Date (Org)', labelBN: 'শাস্তির তারিখ (সংস্থা)', hint: 'PlainDate', defaultVisible: false },
        { key: 'action',           labelEN: 'Action',           labelBN: 'গৃহীত ব্যবস্থা',    hint: 'Plain',     defaultVisible: false },
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        offenseDate:      { en: 'offenseDate' },
        offenseType:      { en: 'offenseType',    bn: 'offenseTypeBN' },
        briefStatement:   { en: 'briefStatement', bn: 'briefStatementBN' },
        offenseDetails:   { en: 'offenseDetails' },
        punishmentRAB:    { en: 'punishmentRAB',  bn: 'punishmentRABBN' },
        punishmentDate:   { en: 'punishmentDate' },
        punishmentMO:     { en: 'punishmentMO',   bn: 'punishmentMOBN' },
        punishmentDateMO: { en: 'punishmentDateMO' },
        action:           { en: 'action' },
        auth:             { en: 'auth' },
        remarks:          { en: 'remarks' },
    };

    plainCellValue(row: DisciplineReportRow, key: string): string {
        const map = ReportDisciplineIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    plainDateCellValue(row: DisciplineReportRow, key: string): string {
        const map = ReportDisciplineIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের কোনো শৃঙ্খলা সংক্রান্ত তথ্য নেই।' : 'No discipline records found for this member.';
    }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        this.disciplineInfoService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    .filter((item: any) => (item.employeeID ?? item.EmployeeID) === employeeId)
                    .map((item: any) => ({
                        offenseDate: item.offenseDate ?? item.OffenseDate ?? null,
                        offenseType: item.offenseType ?? item.OffenseType ?? null, offenseTypeBN: item.offenseTypeBN ?? item.OffenseTypeBN ?? null,
                        briefStatement: item.briefStatementOfOffence ?? item.BriefStatementOfOffence ?? null, briefStatementBN: item.briefStatementOfOffenceBN ?? item.BriefStatementOfOffenceBN ?? null,
                        offenseDetails: item.offenseDetails ?? item.OffenseDetails ?? null,
                        punishmentRAB: item.punishmentTypeRAB ?? item.PunishmentTypeRAB ?? null, punishmentRABBN: item.punishmentTypeRABBN ?? item.PunishmentTypeRABBN ?? null,
                        punishmentDate: item.punishmentDate ?? item.PunishmentDate ?? null,
                        punishmentMO: item.punishmentTypeMotherOrg ?? item.PunishmentTypeMotherOrg ?? null, punishmentMOBN: item.punishmentTypeMotherOrgBN ?? item.PunishmentTypeMotherOrgBN ?? null,
                        punishmentDateMO: item.punishmentDateMotherOrg ?? item.PunishmentDateMotherOrg ?? null,
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load discipline records' });
                this.loading = false;
            },
        });
    }
    ...
    private cellText(row: DisciplineReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportDisciplineIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `discipline-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `discipline-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: DisciplineReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
