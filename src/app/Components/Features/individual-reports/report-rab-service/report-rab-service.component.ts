...
import { formatPartialDate, type DatePrecision } from '@/shared/utils/partial-date.util';
import type {
...
/**
 * Standalone "Service History in RAB" report — one row per previous-RAB-
 * service record for the searched member. Source view
 * (vw_PreviousRABServiceInfo) already resolves unit/wing/branch/appointment
 * names, so no CommonCode resolution is needed here. Mirrors the structure
 * of report-address / report-nominee under individual-reports.
 */
type RabServiceReportRow = {
    ser?: number;
    unit: string | null;       unitBN: string | null;
    wing: string | null;       wingBN: string | null;
    branch: string | null;     branchBN: string | null;
    subBranch: string | null;  subBranchBN: string | null;
    section: string | null;    sectionBN: string | null;
    subSection: string | null; subSectionBN: string | null;
    appointment: string | null; appointmentBN: string | null;
    serviceFrom: string | null; fromPrecision: DatePrecision | null;
    serviceTo: string | null;   toPrecision: DatePrecision | null;
    isActive: boolean;
...
@Component({
    selector: 'app-report-rab-service-individual',
    standalone: true,
    ...
    templateUrl: './report-rab-service.component.html',
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-rab-service.component.scss'],
})
export class ReportRabServiceIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'placement',   labelEN: 'Placement',    labelBN: 'পদায়ন',        hint: 'Composite', defaultVisible: true  },
        ...
        { key: 'status',      labelEN: 'Status',       labelBN: 'অবস্থা',        hint: 'Status',    defaultVisible: true  },
        // Opt-in extras
        { key: 'postingAuth', labelEN: 'Posting Auth', labelBN: 'পদায়ন কর্তৃপক্ষ', hint: 'Plain',   defaultVisible: false },
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        ...
        postingAuth: { en: 'postingAuth' },
        remarks:     { en: 'remarks' },
    };

    plainCellValue(row: RabServiceReportRow, key: string): string {
        const map = ReportRabServiceIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
        ...
        this.previousRABService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    ...
                    .map((item: any) => {
                        const isActive = (item.isCurrentlyActive ?? item.IsCurrentlyActive) === true || (item.isCurrentlyActive ?? item.IsCurrentlyActive) === 1;
                        ...
                        return {
                            unit: item.rabUnitName ?? item.RABUnitName ?? null, unitBN: item.rabUnitNameBN ?? item.RABUnitNameBN ?? null,
                            wing: item.rabWingName ?? item.RABWingName ?? null, wingBN: item.rabWingNameBN ?? item.RABWingNameBN ?? null,
                            branch: item.rabBranchName ?? item.RABBranchName ?? null, branchBN: item.rabBranchNameBN ?? item.RABBranchNameBN ?? null,
                            subBranch: item.rabSubBranchName ?? item.RABSubBranchName ?? null, subBranchBN: item.rabSubBranchNameBN ?? item.RABSubBranchNameBN ?? null,
                            section: item.rabSectionName ?? item.RABSectionName ?? null, sectionBN: item.rabSectionNameBN ?? item.RABSectionNameBN ?? null,
                            subSection: item.rabSubSectionName ?? item.RABSubSectionName ?? null, subSectionBN: item.rabSubSectionNameBN ?? item.RABSubSectionNameBN ?? null,
                            appointment: item.appointmentName ?? item.AppointmentName ?? null, appointmentBN: item.appointmentNameBN ?? item.AppointmentNameBN ?? null,
                            serviceFrom: item.serviceFrom ?? item.ServiceFrom ?? null, fromPrecision: fromP,
                            serviceTo: item.serviceTo ?? item.ServiceTo ?? null, toPrecision: toP,
                            isActive,
                            postingAuth: item.postingAuth ?? item.PostingAuth ?? null,
                            remarks: item.remarks ?? item.Remarks ?? null,
                        } as RabServiceReportRow;
                    });

                // Active first, then most-recent start date.
                shaped.sort((a, b) => {
                    if (a.isActive !== b.isActive) return a.isActive ? -1 : 1;
                    return this.timeOf(b.serviceFrom) - this.timeOf(a.serviceFrom);
                });
    ...
    // ── Cell renderers ─────────────────────────────────────────────────
    placementCrumbParts(row: RabServiceReportRow): { label: string; value: string }[] {
        const bn = this.lang === 'bn';
        const parts = [
            { label: bn ? 'ব্যাটালিয়ন' : 'Battalion',   value: this.codeValue(row.unit, row.unitBN) },
            { label: bn ? 'উইং' : 'Wing',                 value: this.codeValue(row.wing, row.wingBN) },
            { label: bn ? 'শাখা' : 'Branch',              value: this.codeValue(row.branch, row.branchBN) },
            { label: bn ? 'উপশাখা' : 'Sub-Branch',        value: this.codeValue(row.subBranch, row.subBranchBN) },
            { label: bn ? 'সেকশন' : 'Section',            value: this.codeValue(row.section, row.sectionBN) },
            { label: bn ? 'উপ-সেকশন' : 'Sub-Section',     value: this.codeValue(row.subSection, row.subSectionBN) },
        ];
        return parts.filter(p => p.value && p.value !== '-');
    }

    formatPeriod(row: RabServiceReportRow): string {
        const from = this.partial(row.serviceFrom, row.fromPrecision);
        const to = row.isActive ? (this.lang === 'bn' ? 'বর্তমান' : 'Present') : this.partial(row.serviceTo, row.toPrecision);
        if (from === '—' && to === '—') return '-';
        return `${from} – ${to}`;
    }

    displayStatus(row: RabServiceReportRow): string {
        const bn = this.lang === 'bn';
        return row.isActive ? (bn ? 'সক্রিয়' : 'Active') : (bn ? 'পূর্ববর্তী' : 'Previous');
    }

    private partial(value: string | null, precision: DatePrecision | null): string {
        const s = formatPartialDate(value, precision);
        if (s === '—') return s;
        return this.lang === 'bn' ? BanglaNumerals.toBangla(s) : s;
    }

    private cellText(row: RabServiceReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
            case 'Serial':    return this.paddedSer(row.ser);
            case 'Composite': return this.placementCrumbParts(row).map(p => `${p.label}: ${p.value}`).join(' › ') || '-';
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportRabServiceIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                const isMono = isSerial || col.hint === 'Period';
        ...
        saveAs(blob, `rab-service-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `rab-service-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: RabServiceReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
                ...
                case 'Status': return `<td class="td-status">${esc(this.displayStatus(row))}</td>`;
                case 'Composite': {
                    const crumbs = this.placementCrumbParts(row);
                    const crumbHtml = crumbs.map((p, i) => `<span class="addr-part"><span class="addr-label">${esc(p.label)}:</span> <span class="addr-value">${esc(p.value)}</span></span>` + (i < crumbs.length - 1 ? '<span class="addr-sep">&rsaquo;</span>' : '')).join(' ');
                    return `<td class="td-address"><div class="addr-crumb">${crumbHtml || '-'}</div></td>`;
                }
