...
/**
 * Standalone "Promotion History" report — one row per promotion record for
 * the searched member. Source view (vw_PromotionInfoByEmployee) already
 * resolves rank names. Mirrors the structure of report-mo-service.
 */
type PromotionReportRow = {
    ser?: number;
    previousRank: string | null; previousRankBN: string | null;
    promotedRank: string | null; promotedRankBN: string | null;
...
@Component({
    selector: 'app-report-promotion-individual',
    standalone: true,
    ...
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-promotion.component.scss'],
})
export class ReportPromotionIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'previousRank',       labelEN: 'Previous Rank',  labelBN: 'পূর্বের পদবি',   hint: 'Plain',     defaultVisible: true  },
        { key: 'promotedRank',       labelEN: 'Promoted Rank',  labelBN: 'পদোন্নত পদবি',   hint: 'Plain',     defaultVisible: true  },
        { key: 'promotedDate',       labelEN: 'Promotion Date', labelBN: 'পদোন্নতির তারিখ', hint: 'PlainDate', defaultVisible: true  },
        // Opt-in extras
        { key: 'effectivePeriod',    labelEN: 'Effective Period',labelBN: 'কার্যকর মেয়াদ', hint: 'Period',    defaultVisible: false },
        { key: 'probationaryPeriod', labelEN: 'Probation',      labelBN: 'প্রবেশনকাল',     hint: 'Plain',     defaultVisible: false },
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        previousRank:       { en: 'previousRank',       bn: 'previousRankBN' },
        promotedRank:       { en: 'promotedRank',       bn: 'promotedRankBN' },
        promotedDate:       { en: 'promotedDate' },
        probationaryPeriod: { en: 'probationaryPeriod' },
        auth:               { en: 'auth' },
        remarks:            { en: 'remarks' },
    };

    plainCellValue(row: PromotionReportRow, key: string): string {
        const map = ReportPromotionIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    plainDateCellValue(row: PromotionReportRow, key: string): string {
        const map = ReportPromotionIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
        return this.formatDate((row as any)[map.en] as string | null | undefined);
    }
    formatPeriod(row: PromotionReportRow): string {
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের পদোন্নতির তথ্য নেই।' : 'No promotion records found for this member.';
    }
    ...
    get rabSectionTitle(): string { return this.lang === 'bn' ? 'পদোন্নতির ইতিহাস' : 'PROMOTION HISTORY'; }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        this.promotionInfoService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    .filter((item: any) => (item.employeeID ?? item.EmployeeID) === employeeId)
                    .map((item: any) => ({
                        previousRank: item.previousRank ?? item.PreviousRank ?? null, previousRankBN: item.previousRankBN ?? item.PreviousRankBN ?? null,
                        promotedRank: item.promotedRank ?? item.PromotedRank ?? null, promotedRankBN: item.promotedRankBN ?? item.PromotedRankBN ?? null,
                        ...
                        probationaryPeriod: item.probationaryPeriod ?? item.ProbationaryPeriod ?? null,
                        auth: item.auth ?? item.Auth ?? null,
                        remarks: item.remarks ?? item.Remarks ?? null,
                    } as PromotionReportRow));
                shaped.sort((a, b) => this.timeOf(b.promotedDate) - this.timeOf(a.promotedDate));
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load promotion history' });
                this.loading = false;
            },
        });
    }
    ...
    private cellText(row: PromotionReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportPromotionIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                const isMono = isSerial || col.hint === 'Period' || col.hint === 'PlainDate';
        ...
        saveAs(blob, `promotion-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `promotion-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: PromotionReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
