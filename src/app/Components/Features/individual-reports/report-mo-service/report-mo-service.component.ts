...
/**
 * Standalone "Service History in Mother Organization" report — one row per
 * MO service-history record for the searched member. Source view
 * (vw_MOServHistoryByEmployee) already resolves organization / appointment
 * names. Mirrors the structure of report-rab-service / report-nominee.
 */
...
@Component({
    selector: 'app-report-mo-service-individual',
    standalone: true,
    ...
    templateUrl: './report-mo-service.component.html',
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-mo-service.component.scss'],
})
export class ReportMoServiceIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        // Opt-in extras
        { key: 'auth',         labelEN: 'Posting Auth',  labelBN: 'পদায়ন কর্তৃপক্ষ', hint: 'Plain', defaultVisible: false },
    ...
    plainCellValue(row: MoServiceReportRow, key: string): string {
        const map = ReportMoServiceIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের মাতৃ সংস্থায় চাকরির তথ্য নেই।' : 'No mother-organization service records found for this member.';
    }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        this.moServHistoryService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    ...
                    .map((item: any) => {
                        ...
                        } as MoServiceReportRow;
                    });

                // Most-recent start date first.
                shaped.sort((a, b) => this.timeOf(b.serviceFrom) - this.timeOf(a.serviceFrom));
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load service history' });
                this.loading = false;
            },
        });
    }
    ...
    formatPeriod(row: MoServiceReportRow): string {
        const from = this.partial(row.serviceFrom, row.fromPrecision);
        const to = this.partial(row.serviceTo, row.toPrecision);
        if (from === '—' && to === '—') return '-';
        return `${from} – ${to}`;
    }
    ...
    private cellText(row: MoServiceReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportMoServiceIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `mo-service-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `mo-service-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: MoServiceReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
