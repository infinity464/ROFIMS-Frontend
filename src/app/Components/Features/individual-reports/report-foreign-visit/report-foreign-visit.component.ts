...
/**
 * Standalone "Ex-Bangladesh Leave (Foreign Visit)" report — one row per
 * foreign-visit record for the searched member. Source view
 * (vw_ForeignVisitInfoByEmployee) already resolves country / visit-type /
 * reason names. Mirrors the structure of report-promotion.
 */
type ForeignVisitReportRow = {
    ser?: number;
    country: string | null; countryBN: string | null;
    visitType: string | null; visitTypeBN: string | null;
    reason: string | null; reasonBN: string | null;
...
export class ReportForeignVisitIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'visitType', labelEN: 'Visit Type',   labelBN: 'ভ্রমণের ধরন',    hint: 'Plain',  defaultVisible: true  },
        { key: 'reason',    labelEN: 'Reason',       labelBN: 'কারণ',           hint: 'Plain',  defaultVisible: true  },
        { key: 'duration',  labelEN: 'Duration',     labelBN: 'মেয়াদ',         hint: 'Period', defaultVisible: true  },
        // Opt-in extras
        { key: 'documents', labelEN: 'Documents',    labelBN: 'সংশ্লিষ্ট নথি',  hint: 'Plain',  defaultVisible: false },
    ];

    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        country:   { en: 'country',   bn: 'countryBN' },
        visitType: { en: 'visitType', bn: 'visitTypeBN' },
        reason:    { en: 'reason',    bn: 'reasonBN' },
        documents: { en: 'documents' },
    };

    plainCellValue(row: ForeignVisitReportRow, key: string): string {
        const map = ReportForeignVisitIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    formatPeriod(row: ForeignVisitReportRow): string {
        const from = this.formatDate(row.durationFrom);
        const to = this.formatDate(row.durationTo);
        if (from === '-' && to === '-') return '-';
        return `${from} – ${to}`;
    }
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের বিদেশ ভ্রমণের তথ্য নেই।' : 'No foreign visit records found for this member.';
    }
    ...
    get rabSectionTitle(): string { return this.lang === 'bn' ? 'বাংলাদেশের বাইরে ভ্রমণ প্রতিবেদন' : 'EX-BANGLADESH LEAVE (FOREIGN VISIT) REPORT'; }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        this.foreignVisitService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    .filter((item: any) => (item.employeeID ?? item.EmployeeID) === employeeId)
                    .map((item: any) => ({
                        country: item.nameOfCountry ?? item.NameOfCountry ?? null, countryBN: item.nameOfCountryBN ?? item.NameOfCountryBN ?? null,
                        visitType: item.visitType ?? item.VisitType ?? null, visitTypeBN: item.visitTypeBN ?? item.VisitTypeBN ?? null,
                        reason: item.reasonForVisiting ?? item.ReasonForVisiting ?? null, reasonBN: item.reasonForVisitingBN ?? item.ReasonForVisitingBN ?? null,
                        durationFrom: item.durationFrom ?? item.DurationFrom ?? null,
                        durationTo: item.durationTo ?? item.DurationTo ?? null,
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load foreign visit records' });
                this.loading = false;
            },
        });
    }
    ...
    private cellText(row: ForeignVisitReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportForeignVisitIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `foreign-visit-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `foreign-visit-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: ForeignVisitReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
