...
import { RankConfirmationInfoService } from '@/services/rank-confirmation-info.service';
...
/**
 * Standalone "Rank Confirmation" report — one row per rank-confirmation
 * record for the searched member. The entity stores a rank CodeId, so the
 * report resolves it to a name via CommonCode ('MotherOrgRank' + 'Rank').
 */
type RankConfirmReportRow = {
    ser?: number;
    presentRank: string | null; presentRankBN: string | null;
...
@Component({
    selector: 'app-report-rank-confirmation-individual',
    standalone: true,
    ...
    templateUrl: './report-rank-confirmation.component.html',
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-rank-confirmation.component.scss'],
})
export class ReportRankConfirmationIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'presentRank',     labelEN: 'Confirmed Rank',   labelBN: 'নিশ্চিতকৃত পদবি',     hint: 'Plain',     defaultVisible: true  },
        { key: 'rankConfirmDate', labelEN: 'Confirmation Date',labelBN: 'নিশ্চিতকরণের তারিখ',  hint: 'PlainDate', defaultVisible: true  },
        // Opt-in extras
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        presentRank:     { en: 'presentRank', bn: 'presentRankBN' },
        rankConfirmDate: { en: 'rankConfirmDate' },
        auth:            { en: 'auth' },
        remarks:         { en: 'remarks' },
    };

    plainCellValue(row: RankConfirmReportRow, key: string): string {
        const map = ReportRankConfirmationIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    plainDateCellValue(row: RankConfirmReportRow, key: string): string {
        const map = ReportRankConfirmationIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের পদবি নিশ্চিতকরণের তথ্য নেই।' : 'No rank confirmation records found for this member.';
    }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        forkJoin({
            ranks: this.rankConfirmationService.getByEmployeeId(employeeId).pipe(catchError(() => of([] as any[]))),
            moRank: this.commonCodeService.getAllActiveCommonCodesType('MotherOrgRank').pipe(catchError(() => of([] as any[]))),
            rank: this.commonCodeService.getAllActiveCommonCodesType('Rank').pipe(catchError(() => of([] as any[]))),
        }).subscribe({
            next: ({ ranks, moRank, rank }) => {
                const rankMap = this.buildCodeMap([...(moRank ?? []), ...(rank ?? [])]);
                const shaped = (ranks ?? [])
                    .filter((item: any) => (item.employeeId ?? item.EmployeeId) === employeeId)
                    .map((item: any) => {
                        const rankId = item.presentRank ?? item.PresentRank ?? null;
                        const r = rankId != null ? rankMap.get(Number(rankId)) : undefined;
                        return {
                            presentRank: r?.en ?? (rankId != null ? String(rankId) : null),
                            presentRankBN: r?.bn ?? null,
                        ...
                        } as RankConfirmReportRow;
                    });
                shaped.sort((a, b) => this.timeOf(b.rankConfirmDate) - this.timeOf(a.rankConfirmDate));
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load rank confirmation records' });
                this.loading = false;
            },
        });
    }
    ...
    private cellText(row: RankConfirmReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportRankConfirmationIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        saveAs(blob, `rank-confirmation-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `rank-confirmation-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: RankConfirmReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
