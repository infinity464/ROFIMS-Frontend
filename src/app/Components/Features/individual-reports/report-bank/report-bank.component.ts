...
/**
 * Standalone "Bank Account" report — one row per bank account for the
 * searched member. Source view (vw_BankAccInfoByEmployee) already resolves
 * bank / branch / account names.
 */
type BankReportRow = {
    ser?: number;
    bankName: string | null; bankNameBN: string | null;
    branchName: string | null; branchNameBN: string | null;
    accountName: string | null; accountNameBN: string | null;
...
@Component({
    selector: 'app-report-bank-individual',
    standalone: true,
    ...
    templateUrl: './report-bank.component.html',
    styleUrls: ['../../employee-reports/report-theme.scss', '../../employee-reports/report-card-mtr.scss', './report-bank.component.scss'],
})
export class ReportBankIndividualComponent implements OnInit, OnChanges {
    L = REPORT_LABELS;
    ...
    columnCatalog: { key: string; labelEN: string; labelBN: string; hint: string; defaultVisible: boolean }[] = [
        ...
        { key: 'bankName',      labelEN: 'Bank',           labelBN: 'ব্যাংক',        hint: 'Plain',  defaultVisible: true  },
        { key: 'branchName',    labelEN: 'Branch',         labelBN: 'শাখা',          hint: 'Plain',  defaultVisible: true  },
        { key: 'accountName',   labelEN: 'Account Name',   labelBN: 'হিসাবের নাম',   hint: 'Plain',  defaultVisible: true  },
        { key: 'accountNumber', labelEN: 'Account Number', labelBN: 'হিসাব নম্বর',   hint: 'Plain',  defaultVisible: true  },
        // Opt-in extras
    ...
    private static readonly plainColumnPropertyMap: Record<string, { en: string; bn?: string }> = {
        bankName:      { en: 'bankName',      bn: 'bankNameBN' },
        branchName:    { en: 'branchName',    bn: 'branchNameBN' },
        accountName:   { en: 'accountName',   bn: 'accountNameBN' },
        accountNumber: { en: 'accountNumber' },
        remarks:       { en: 'remarks' },
    };

    plainCellValue(row: BankReportRow, key: string): string {
        const map = ReportBankIndividualComponent.plainColumnPropertyMap[key];
        if (!map) return '-';
    ...
    get noRecordsMessage(): string {
        return this.lang === 'bn' ? 'এই সদস্যের কোনো ব্যাংক হিসাবের তথ্য নেই।' : 'No bank account records found for this member.';
    }
    ...
    get rabSectionTitle(): string { return this.lang === 'bn' ? 'ব্যাংক হিসাব প্রতিবেদন' : 'BANK ACCOUNT REPORT'; }
    ...
    private fetchForEmployee(employeeId: number, lookupRow: DynamicReportRow): void {
        ...
        this.bankAccInfoService.getViewByEmployeeId(employeeId).subscribe({
            next: (list) => {
                const shaped = (list ?? [])
                    .filter((item: any) => (item.employeeID ?? item.EmployeeID) === employeeId)
                    .map((item: any) => ({
                        bankName: item.bankName ?? item.BankName ?? null, bankNameBN: item.bankNameBN ?? item.BankNameBN ?? null,
                        branchName: item.branchName ?? item.BranchName ?? null, branchNameBN: item.branchNameBN ?? item.BranchNameBN ?? null,
                        accountName: item.accountName ?? item.AccountName ?? null, accountNameBN: item.accountNameBN ?? item.AccountNameBN ?? null,
                        accountNumber: item.accountNumber ?? item.AccountNumber ?? null,
            ...
            error: (err) => {
                console.error(err);
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to load bank account records' });
                this.loading = false;
            },
        });
    }

    private cellText(row: BankReportRow, col: { key: string; hint: string }): string {
        switch (col.hint) {
    ...
    private formatPostingStatus(raw: unknown): string {
        ...
        if (!s) return '-';
        const mapped = ReportBankIndividualComponent.statusDisplayMap[s];
    ...
    private async exportRabWord(): Promise<void> {
        ...
        const dataRows: TableRow[] = this.list.map((row, idx) => {
            ...
            const cells: TableCell[] = visibleCols.map(col => {
                ...
                const run = new TextRun({ text: wsafe(this.cellText(row, col)), font: isSerial ? mono : sans, size: isSerial ? S.name : S.body, ...bnRunExtras(isSerial ? S.name : S.body), bold: isSerial, color: isSerial ? C.gray : C.black, ...(isBn ? {} : { characterSpacing: isSerial ? 8 : 0 }) });
        ...
        saveAs(blob, `bank-account-report_${this.lang}.docx`);
    }
    ...
    private exportRabExcel(): void {
        ...
        XLSX.writeFile(wb, `bank-account-report_${this.lang}.xlsx`);
    }
    ...
    private buildRabPrintHtml(): string {
        ...
        const renderCell = (row: BankReportRow, col: { key: string; hint: string }): string => {
            switch (col.hint) {
