/**
 * Note-sheet id URL codec — COSMETIC obfuscation only.
 *
 * ⚠️ This is NOT a security control. It only stops a casual user from editing the
 * numeric id in the URL bar (e.g. `?id=6137` → `?id=6138`). A determined user can
 * still read the real id from the Network tab and call the API directly — the
 * encryption key lives in this bundle. Resource access is enforced SERVER-SIDE
 * (NoteSheetInfoController → CheckNoteSheetAccess). Do not rely on this for auth.
 *
...
/** Encode a note-sheet id into an opaque URL token (e.g. 6137 → "kX3Rf..."). */
export function encodeNoteSheetId(id: number | string | null | undefined): string {
...
export function decodeNoteSheetId(token: string | number | null | undefined): number | null {
